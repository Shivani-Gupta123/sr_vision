...
let model = {
    ...
    "url": "/bin/tata/scipJobPostingsFilterServlet",
    "urlSearch": "/bin/tata/scipJobPostingsSearchServlet",
...
window.addEventListener("load", async function () {
    ...
    jobFormat = document.getElementsByClassName("filterSec")[0].dataset.format;
...
function makeRequest(url, postParameters) {
    return new Promise(function (resolve, reject) {
        ...
        model.lastUrl = url;
        xhttp.send(postParameters + ((postParameters.length > 0) ? "&jobFormat=" : "jobFormat=") + jobFormat);
    });
}
...
function jobPostingList(jobListing) {
    if (jobListing != null) {
        ...
        var jobLength = 3; //for disclaimer strip
        if (jobListing.length > 0 && start === 1) {
            ...
            let lowerContent = createHtmlForJobList(jobLength, jobListing.length, jobListing);
...
function createHtmlForJobList(startNum, endNum, jobListing) {
    ...
    for (let i = startNum; i < endNum; i++) {
        ...
        let redirectionUrl = jobDetailUrl ? jobDetailUrl + "?jobId=" + encodeURIComponent(jobID) + "&company=" + encodeURIComponent(companyName) + "&jobTitle=" + encodeURIComponent(jobTitle) + "&location=" + encodeURIComponent(jobLoc) + "&jobFormat=" + encodeURIComponent(jobFormat) : "";
