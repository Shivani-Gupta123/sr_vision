...
let clearText = "Clear"
let clearFilterDiv = document.getElementById("clearFilter");

//template to create labels
let checkboxLabel = document.createElement("LABEL");
checkboxLabel.innerHTML = "<div class='inpCont'><input type='checkbox' class='checkBox'/></div>";
let radioLabel = document.createElement("LABEL");
radioLabel.innerHTML = "<div class='inpCont'><input type='radio' class='radioBox' name='date'/></div>";
let jobCardTemplate = document.createElement("DIV");
jobCardTemplate.classList.add("bgChange");
jobCardTemplate.innerHTML = '<div class="container"><div class="row"><div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 jobCardList "><a class="anchorLinkCol" target="_blank" href=""><div class="col-xs-12 col-sm-2 col-md-2 col-lg-2 noPadding imgContainer"><img src="" alt="Company logo" class="img-responsive"></div>' +
    '<div class="col-xs-12 col-sm-8 col-md-8 col-lg-9 col-sm-offset-right-2 col-md-offset-right-2 col-lg-offset-right-1 noPadding"><div class="cardListDetails">' +
    '<div class="jobTitle"></div>' +
    '<div class="details"><div></div>' +
    '<div></div>' + ((window.innerWidth < tabletBreakpoint) ? "<span>/</span>" : "") +
    '<div class = "jobDate"></div></div>' +
...
//Object model of current filter status
let customSelectDiv = document.getElementsByClassName("custom-select");
let filterText = new Map();
filterText.set("companies", "By Company");
filterText.set("Date", "By Date");
filterText.set("locations", "By Location");
filterText.set("jobTitles", "By Title");
function pushToDataLayer(criteriaOld, value) {
    let criteria = criteriaOld.slice(3);
    dataLayer.push({
        event: "GAEvent" + criteria + "Filter",
        eventCategory: "JobListing " + criteria + " Filter",
        eventAction: "Select " + criteria + " filter radio button",
        eventLabel: decodeURIComponent(value)
    });
}
//to map filter name to filter div
let customSelect = new Map();
for (let div of customSelectDiv) {
    customSelect.set(div.dataset.criteria, div);
}
...
let model = {
    ...
    {
        "Date": {
            "filterList": ["Anytime", "Last 24 hours", "Last 7 days", "Last 30 days"],
            ...
            clearDelta() {
                this.delta.clear();
                model.changedFilters = false;
            },
            apply(value) {
                if (!this.selectedFilters.has(value)) {
                    pushToDataLayer("By Date", value);
                }
    ...
    "url": "/bin/tata/jobPostingsFilterServlet",
    "urlSearch": "/bin/tata/jobPostingsSearchServlet",
    ...
    "maximumJobCount": 0,
    "currentJobCount": 0,
    //Post parameters version of selected filters
    "lastQuery": null,
    "lastUrl": "",
    "changedFilters": false,
    //create post function body from selected filters
    parametrize() {
        let postParameters = "";
        for (let criteria in model.filters) {
            if (model.filters[criteria].selectedFilters.size > 0) {
                if (criteria !== "Date") {
                    postParameters = postParameters + "&" + criteria + "=" + JSON.stringify((Array.from(model.filters[criteria].selectedFilters)).map(encodeURIComponent));
                }
                else {
                    postParameters = postParameters + "&" + "selectedDate=" + decodeURIComponent(Array.from(model.filters["Date"].selectedFilters)[0].toLowerCase());
                }

            }
        }
        return postParameters.slice(1);
    },
    //populate filterList
    applyResponse(responseFilters) {
        changedFilters = false;
        for (let filter of Reflect.ownKeys(responseFilters)) {
            if ((responseFilters[filter])) {
                model.filters[filter].filterList = responseFilters[filter];
            }
        }
    },
    ...
    applyDelta() {
        model.changedFilters = false;
        for (let field of Reflect.ownKeys(this.filters)) {
            if (this.filters[field].delta.size > 0) {
                for (let element of this.filters[field].delta) {
                    this.filters[field].apply(element);

                    this.filters[field].delta.delete(element);
                }
            }
        }
        sessionStorage.setItem("loc",JSON.stringify([...this.filters["locations"].selectedFilters]));
    },
    //for applied filters based on url parameters
    getUrlParametersPost() {
        let urlGet = new URL(window.location.href);
        let country = urlGet.searchParams.get("country") ? urlGet.searchParams.get("country").toLowerCase() : null;
        let city = urlGet.searchParams.get("city") ? urlGet.searchParams.get("city").toLowerCase() : null;
        if (city) {
            let combined = (city + ", " + country)
            combined = combined.toLowerCase();
            for (let location of this.filters["locations"].filterList) {
                if (location.toLowerCase() == combined) {
                    this.filters["locations"].selectedFilters.add(location);
                }
            }
            ...
            for (let location of this.filters["locations"].filterList) {
                if (location.split(',')[1].toLowerCase().includes(country)) {
                    this.filters["locations"].selectedFilters.add(location);
                }
            }
        }
    ...
    getUrlParametersPre(){
        let urlGet = new URL(window.location.href);
        let date = urlGet.searchParams.get("date") ? urlGet.searchParams.get("date").toLowerCase() : null;
        model.filters.Date.selectedFilters.clear();
        if (date){
            switch(date){
                case 'a':
                    model.filters.Date.selectedFilters.add("Anytime");
                    break;
                case 'd':
                    model.filters.Date.selectedFilters.add("Last 24 hours");
                    break;
                case 'w':
                    model.filters.Date.selectedFilters.add("Last 7 days");
                    break;
                case 'm':
                    model.filters.Date.selectedFilters.add("Last 30 days");
            }
        }
        
    }
}
//Populates object model with non date filters
let filtersArray = ["companies", "locations", "jobTitles"];
for (let criteria of filtersArray) {
    let dataObject = {
        "filterList": [],
        "selectedFilters": new Set(),
        "delta": new Set(),
        update(value) {
            if (this.delta.has(value)) {
                this.delta.delete(value);
            }
            else {
                this.delta.add(value);
            }
        ...
        apply(value) {
            if (this.selectedFilters.has(value)) {
                ...
                this.selectedFilters.add(value);
                pushToDataLayer(filterText.get(criteria), value);
            }
        }
    }
    model.filters[criteria] = dataObject;
}

//------------------------------------------------------------------------------ Event Listeners ------------------------------------------------------------------------//

//Calling ajax on page load and applying results
window.addEventListener("load", async function () {
    for (let key of Reflect.ownKeys(model.filters)) {

        let div = customSelect.get(key).children[0];
        let innerDiv = customSelect.get(key).getElementsByClassName("scroll-bar-div")[0];
        innerDiv.addEventListener("click", function (e) {
            let childCheckbox = e.target;
            if ((childCheckbox.classList.contains("checkBox")) || childCheckbox.classList.contains("radioBox")) {
                pushToModel(childCheckbox.value, key);
                model.changedFilters = true;
            }
        }, false);
        for (let button of document.getElementsByClassName("btnCancel")) {
            button.addEventListener("click", clearCancelfunction);
        }
        for (let button of document.getElementsByClassName("btnApply")) {
            button.addEventListener("click", applySelected)
        }

        div.addEventListener("click", function (e) {
            this.nextElementSibling.classList.toggle("select-hide");
            this.classList.toggle("select-arrow-active");
            let drpdnSearch = this.nextElementSibling.getElementsByClassName("dropdnSearchIp");
            let activedrpdown = this.nextElementSibling.getElementsByClassName("scroll-bar-div")[0];
            if (drpdnSearch.length > 0) {
                if (drpdnSearch[0].value !== "") {
                    drpdnSearch[0].value = "";
                    this.nextElementSibling.getElementsByClassName("noSuggest")[0].classList.add("noDisplay");
                }
                for (i = 2; i < activedrpdown.children.length; i++) {
                    activedrpdown.children[i].classList.remove("noDisplay");
                }
            }
        });
    ...
    model.getUrlParametersPre();
    await reset(model.parametrize());
    model.getUrlParametersPost();
    await reset(model.parametrize());  
    window.scrollTo(0,0);
});
...
document.addEventListener("click", (e) => {
    let mobFilterSec = document.getElementsByClassName("wrapper");
    if ((!mobFilterSec[0].classList.contains("hidden-xs")) || (document.getElementsByClassName("select-arrow-active").length > 0)) {
        if (e.target.closest(".select-items") || e.target.closest(".cancelParentArea")) {
            //do nothing
        }
        else if (e.target.closest(".select-selected")) {
            //apply and prevent opening of dropdown
            let deltaSize = 0;
             for (let key of Reflect.ownKeys(model.filters)) {
                deltaSize += model.filters[key].delta.size;
            }
            //check if filters are applied
            if (deltaSize > 0) {
                if ((document.getElementsByClassName("select-arrow-active")).length > 0) {
                    if (model.changedFilters) {
                        e.stopPropagation();
                        applySelected();
                        checkActiveDropdn();
                    }
            ...
            else if (!e.target.closest(".select-arrow-active")){
                closeAllSelect();
            }

        }
        else if (e.target.closest(".actionItem") || e.target.closest(".anchorLinkCol")) {
            //cancel current and close dropdown

            if (e.target.closest(".anchorLinkCol")) {
                e.preventDefault();
                applySelected();
            }
            for (let key of Reflect.ownKeys(model.filters)) {
                model.filters[key].clearDelta();
            }
            checkActiveDropdn();
            closeAllSelect();
            mobFilterSec[0].classList.add("hidden-xs");
        }
        else {
            //outside click
            if (model.changedFilters) {
...
//load more jobs 
document.getElementById("loadMrJob").addEventListener("click", async function () {
    start = start + count;
    let result = await makeRequest(model.lastUrl, "filtersFlag=False&start=" + start + "&" + model.lastQuery);
    result = result.response;
    let currentScroll = document.documentElement.scrollTop;
    jobPostingList(result.jobPostings);
    window.scrollTo(0,currentScroll);

});
//---------this function is used on enter key press in search box--------------------
document.getElementById("searchInputJob").addEventListener("keypress", function (e) {
    if (event.keyCode == 13 || event.which == 13) {
        searchData();
    }
});
//---------this function is used on Search icon press in search box--------------------
document.getElementById("jobSearchIcon").addEventListener("click", function (e) {
    searchData();
});
...
//Getting data from servlet and regenerating page
async function reset(postParameters) {
    // await code here
    let result = null;
    start = 1;
    postParameters = postParameters + "&currentFilter=" + model.currentFilter;
    if (postParameters != model.lastQuery) {
        try {

            result = await makeRequest(model.url, postParameters);
            result = result.response;
            model.applyResponse(result.jobFilters);
            customDropDownWithDynamic();
...
function makeRequest(url, postParameters) {
    return new Promise(function (resolve, reject) {
        ...
        xhttp.onload = function () {
            try {
                if (this.status == 200) {
                    ...
                    totalCount = response.response.totalJobPostingsCount;
                    resolve(response);
                } else {
                    reject({
                        status: this.status,
        ...
        model.lastQuery = postParameters;
        var removefilterStart;
        var poststringStart = /(filtersFlag=(True|False)&start=\d+&)/;
        var filterStart = /filtersFlag=(True|False)&start=\d+&/g;
        var checkFirstString = model.lastQuery.match(poststringStart);
        if(checkFirstString){
            removefilterStart = model.lastQuery.replace(filterStart, '');
            removefilterStart = checkFirstString[0] + removefilterStart;
        }else{
            removefilterStart = model.lastQuery;
        }
        model.lastUrl = url;
        xhttp.send(removefilterStart);
    });
}

//function to apply response to filters
function customDropDownWithDynamic() {
    for (let key of Reflect.ownKeys(model.filters)) {

        let selElmnt = new Set(model.filters[key].filterList);
        selElmnt = new Set([...selElmnt, ...model.filters[key].selectedFilters]);
        if (selElmnt) {
            let div = customSelect.get(key).children[0];

            const div2 = customSelect.get(key).children[1];
            let innerDiv = div2.children[0];

            while (innerDiv.firstChild) {
                innerDiv.removeChild(innerDiv.firstChild);
            }
            divSearch = document.createElement("div");
            divSearch.setAttribute("class", "dropdnSearch");
            divSearch.innerHTML = "<input type='text' placeholder='Search' class='dropdnSearchIp'></input>";
            let noSuggestion = document.createElement("div");
            noSuggestion.setAttribute("class", "noDisplay noSuggest");
            noSuggestion.innerText = "No results found.";
            if (div.parentNode.dataset.criteria !== "Date") {
                innerDiv.appendChild(divSearch);
                innerDiv.appendChild(noSuggestion);
                divSearch.addEventListener("keyup", function (e) {
                    searchDropdown(key, model.filters[key].filterList);
                });
            }

            for (let label of selElmnt) {
                let labelEl;
                if (key == "Date") {
                    labelEl = radioLabel.cloneNode(true);
                } else {
                    labelEl = checkboxLabel.cloneNode(true);
                }
                labelEl.children[0].children[0].value = label;
                labelEl.children[0].appendChild(document.createTextNode(label));
                innerDiv.appendChild(labelEl);
            }
            if (selElmnt.length == 0) {
                divNoRes = document.createElement("DIV");
                divNoRes.setAttribute("class", "divNoResult");
                divNoRes.innerText = "No results found.";
                div2.appendChild(divNoRes);
            }
    ...
    let dropdownsList = document.getElementsByClassName("scroll-bar-div");
    for (let elem of dropdownsList) {
...
//logic fr displaying jobs
function jobPostingList(jobListing) {
    if (jobListing != null) {
        let btnLoadMore = document.getElementsByClassName("btnLoadMore")[0];
        let joblistInitialData = document.getElementById("joblistInitialData");
        let noLoadMore =  document.getElementsByClassName("noLoadMore")[0];
        let disclaimerStrip = document.getElementsByClassName("strip");
        let jobAlertStrip = document.getElementsByClassName("jobAlertStrip");
        let loadMoreData = document.getElementById("loadMoredata");
        let joblistSecData = document.getElementById("joblistSecData");
        let joblistMiddleData = document.getElementById("joblistMiddleData");
        var jobLength = 3;
        var middlejob = 6;
        if (jobListing.length > 0 && start === 1) {
            while (loadMoreData.firstChild) {
                loadMoreData.removeChild(loadMoreData.firstChild);
            }
            if (jobListing.length < jobLength) {
                jobLength = jobListing.length;
                btnLoadMore.style.display = "none";
                noLoadMore.style.display = "block";
            }
            if(jobListing.length < middlejob){
                middlejob = jobListing.length
                btnLoadMore.style.display = "none";
                noLoadMore.style.display = "block";
              }
              if (jobListing.length <= jobLength) {
                if (jobAlertStrip.length > 0) {
                    jobAlertStrip[0].style.marginBottom="15px"
                }
            }else{
                if (jobAlertStrip.length > 0) {
                    jobAlertStrip[0].style.marginBottom=""
                }
            }
            let upperContent = createHtmlForJobList(0, jobLength, jobListing);
            while (joblistInitialData.firstChild) {
                joblistInitialData.removeChild(joblistInitialData.firstChild);
            }
            while (joblistSecData.firstChild) {
                joblistSecData.removeChild(joblistSecData.firstChild);
            }
            for (let element of upperContent) {
                joblistInitialData.appendChild(element);
            }
            let middleContent = createHtmlForJobList(jobLength, middlejob, jobListing);
            while (joblistMiddleData.firstChild) {
                joblistMiddleData.removeChild(joblistMiddleData.firstChild);
            }
            for (let element of middleContent) {
                joblistMiddleData.appendChild(element);
              
            }
            let lowerContent = createHtmlForJobList(middlejob, jobListing.length, jobListing);
            for (let element of lowerContent) {
                joblistSecData.appendChild(element);
            }


            document.getElementsByClassName("noRes")[0].style.display = "none";
            if (disclaimerStrip.length > 0) {
                disclaimerStrip[0].style.display = "block";

            }
            if (jobAlertStrip.length > 0) {
                jobAlertStrip[0].style.display = "block";
            }
            btnLoadMore.style.display = "block";
            noLoadMore.style.display = "none";
            if (jobListing.length > count) {
                btnLoadMore.style.display = "block";
                noLoadMore.style.display = "none";
            } else {
                btnLoadMore.style.display = "none";
            ...
            abc.setAttribute("class", "loadmore");
            let loadMoreDataContent = createHtmlForJobList(0, jobListing.length, jobListing);
            for (let Card of loadMoreDataContent) {
                abc.appendChild(Card);
            }
            loadMoreData.appendChild(abc);
            if (totalCount <= (start + count-1)) {
                btnLoadMore.style.display = "none";
                noLoadMore.style.display = "block";
            }
        }
            ...
            document.getElementsByClassName("noRes")[0].style.display = "block";
            if (disclaimerStrip.length > 0) {
                disclaimerStrip[0].style.display = "none";
            }
            if (jobAlertStrip.length > 0) {
                jobAlertStrip[0].style.display = "none";
            }
            btnLoadMore.style.display = "none";
            noLoadMore.style.display = "block";
        }
...
//creation of jobs cards
function createHtmlForJobList(startNum, endNum, jobListing) {
    let jobList = [];
    let jobTitle, jobID, jobLoc, jobdesc, companyName, companyLogo, jobDetailUrl, jobDate;
    for (let i = startNum; i < endNum; i++) {
        jobTitle = jobListing[i].jobTitle;
        jobID = jobListing[i].jobId;
        jobLoc = jobListing[i].location;
        jobdesc = jobListing[i].shortDescription ? jobListing[i].shortDescription : "";
        jobDate = jobListing[i].publishedDate;
        companyName = jobListing[i].companyName;
        companyLogo = "" + jobListing[i].companyLogo;
        jobDetailUrl = document.getElementsByClassName("detailPageUrl")[0].getAttribute("href");
        let redirectionUrl = jobDetailUrl ? jobDetailUrl + "?jobId=" + encodeURIComponent(jobID) + "&company=" + encodeURIComponent(companyName) + "&jobTitle=" + encodeURIComponent(jobTitle) + "&location=" + encodeURIComponent(jobLoc) : "";

        if (jobdesc.length >= 250) {
            jobdesc = jobListing[i].shortDescription.substring(0, 250) + '...';
        }
        let Card = jobCardTemplate.cloneNode(true);
        Card.getElementsByClassName("anchorLinkCol")[0].href = redirectionUrl;
        Card.getElementsByTagName("IMG")[0].src = companyLogo;
        Card.getElementsByTagName("IMG")[0].alt = "Company logo";
        Card.getElementsByClassName("jobTitle")[0].innerText = jobTitle;
        Card.getElementsByClassName("details")[0].children[0].innerText = companyName;
        Card.getElementsByClassName("details")[0].children[1].innerText = jobLoc;
        Card.getElementsByClassName("jobDate")[0].innerText = jobDate;
        Card.getElementsByClassName("jobDesc")[0].innerText = jobdesc;
        jobList.push(Card);
...
//updates html of selected dropdowns
function checkActiveDropdn() {
    filterCount = 0;
    for (let key of Reflect.ownKeys(model.filters)) {
        let dropDownInnerDiv = customSelect.get(key).getElementsByClassName("select-selected")[0];
        let cancelBtns = customSelect.get(key).getElementsByClassName("btnCancel");
        let badges = customSelect.get(key).getElementsByClassName("badge");
        let checkBox = []
        if (key !== "Date") {
            checkbox = customSelect.get(key).getElementsByClassName("checkBox");
        }
        else {
            checkbox = customSelect.get("Date").getElementsByClassName("radioBox");
        }
        if (model.filters[key].selectedFilters.size > 0) {
            if (checkbox.length > 0) {
                customSelect.get(key).classList.add("active");
                if (model.filters[key].selectedFilters.size == 1) {

                    if ([...model.filters[key].selectedFilters][0].length > 15 && window.innerWidth >= mobileBreakpoint) {
                        dropdnVal = [...model.filters[key].selectedFilters][0].substring(0, 15) + " " + '...';
                        dropDownInnerDiv.firstChild.nodeValue = dropdnVal;
                    } else {
                        dropDownInnerDiv.firstChild.nodeValue = [...model.filters[key].selectedFilters][0];
                    }
                    if (badges.length > 0) {
                        badges[0].style.display = "none";
                    }
                } else if (model.filters[key].selectedFilters.size > 1) {
                    dropDownInnerDiv.firstChild.nodeValue = filterText.get(key);
                    badges[0].style.display = "inline-block";
                    badges[0].innerText = model.filters[key].selectedFilters.size;
                }
                filterCount = filterCount + model.filters[key].selectedFilters.size;
                if (cancelBtns.length > 0) {
                    cancelBtns[0].innerText = clearText;
                }



            } else {
                if (customSelect.get(key).classList.contains("active")) {
                    dropDownInnerDiv.firstChild.nodeValue = filterText.get(key);
                    customSelect.get(key).classList.remove("active");
                }
                if (cancelBtns.length > 0) {
                    cancelBtns[0].innerText = clearText;
                }
        ...
        } else if (model.filters[key].selectedFilters.size == 0) {
            if (badges.length > 0) {
                badges[0].style.display = "none";
            }
            if (customSelect.get(key).classList.contains("active")) {
            ...
            if (cancelBtns.length > 0) {
                cancelBtns[0].innerText = cancelText;
            }
        }
        for (let checkboxLabel of checkbox) {
            if (model.filters[key].selectedFilters.has(checkboxLabel.value)) {
    ...
    if (filterCount > 0) {
        clearFilterDiv.style.display = "block";
        clearFilterDiv.getElementsByClassName("badge")[0].style.display = "inline-block";
        clearFilterDiv.getElementsByClassName("badge")[0].innerText = filterCount;
        if (window.innerWidth < tabletBreakpoint) {
            document.getElementsByClassName("mobfilterbadge")[0].style.display = "inline-block";
            document.getElementsByClassName("mobfilterbadge")[0].innerText = filterCount;
        }
    } else {
        clearFilterDiv.style.display = "none";
        clearFilterDiv.getElementsByClassName("badge")[0].style.display = "none";
        if (window.innerWidth <= 786) { //TODO make common variable
            document.getElementsByClassName("mobfilterbadge")[0].style.display = "none";
        }
    }

}
//pushes from delta to selected
async function applySelected() {
    model.applyDelta();
    jobSearchTerm = document.getElementById("searchInputJob").value;
    closeAllSelect(this);
    checkActiveDropdn();

    if (jobSearchTerm !== "") {
        document.getElementById("searchInputJob").value = "";
        prevSearchVal = "";
    }
    reset(model.parametrize());
}

//Pushing clicked element to delta set in model
function pushToModel(value, criteria) {

    model.filters[criteria].update(value);
    while (document.getElementById("loadMoredata").firstChild) {
        document.getElementById("loadMoredata").removeChild(document.getElementById("loadMoredata").firstChild);
    }
    start = 1;
    model.currentFilter = (criteria === "Date" ? "selectedDate" : criteria);
    sessionStorage.setItem("loc", JSON.stringify([...model.filters.locations.selectedFilters]));
}

//on clicking clear or cancel button
function clearCancelfunction() {
    let currDropdn = this.parentNode.parentNode.parentNode.dataset.criteria;
    model.currentFilter = (currDropdn === "Date" ? "selectedDate" : currDropdn);
    let btnCancel = customSelect.get(currDropdn).querySelectorAll(".btnCancel")[0];
    let needToReset = false;
    if (btnCancel.innerText === cancelText) {
        for (let key of Reflect.ownKeys(model.filters)) {
            model.filters[key].clearDelta();
        }
    }
        ...
        needToReset = true;
        model.filters[currDropdn].selectedFilters.clear();
        model.filters[currDropdn].clearDelta();


        if (btnCancel.innerText == clearText) {
...
//on search for data in main search bar
async function searchData() {
    let tempStorage = model.lastQuery;
    model.lastQuery = tempStorage;
    clearAllFilters();
    start = 1;
    while (document.getElementById("loadMoredata").firstChild) {
        document.getElementById("loadMoredata").removeChild(document.getElementById("loadMoredata").firstChild);
    }
    let jobSearchTerm = document.getElementById("searchInputJob").value;
    jobSearchTerm = jobSearchTerm.replace(/\s\s+/g, ' ');
    if (jobSearchTerm == " ") jobSearchTerm = "";
    if (!jobSearchTerm == "") {
        let postParameters = "searchTerm=" + encodeURIComponent(jobSearchTerm);
        if (postParameters != model.lastQuery) {
            try {
                let result = await makeRequest(model.url, "");
                result = result.response;
                model.applyResponse(result.jobFilters);
                customDropDownWithDynamic();
                checkActiveDropdn();
                result = await makeRequest(model.urlSearch, postParameters);
                result = result.response;
                errorStatusCode(false);
                jobPostingList(result.jobPostings);
            }
...
function clearAllFilters() {
    for (let key of Reflect.ownKeys(model.filters)) {
        model.filters[key].selectedFilters.clear();
        model.filters[key].clearDelta();
    }
    closeAllSelect();
    checkActiveDropdn();
...
function searchDropdown(key, companylist) {
    let currSearch = key;
    let internalSearch = customSelect.get(currSearch).getElementsByClassName("dropdnSearchIp")[0].value;
    let noSuggestionsDiv = customSelect.get(currSearch).getElementsByClassName("noSuggest")[0];
    let scrollBarDiv = customSelect.get(currSearch).getElementsByClassName("scroll-bar-div")[0];
    for (i = 0; i < companylist.length; i++) {
        //if (companylist[i].substr(0, internalSearch.length).toUpperCase() == internalSearch.toUpperCase()) {
        //To check whole string instead of just starting
        if (companylist[i].toUpperCase().includes(internalSearch.toUpperCase()) ) {
            scrollBarDiv.children[i + 2].classList.remove("noDisplay");//First two elements are not filters

        } else {
            scrollBarDiv.children[i + 2].classList.add("noDisplay");
        }
    }
    if (scrollBarDiv.querySelectorAll("label.noDisplay").length === companylist.length) {
        noSuggestionsDiv.classList.remove("noDisplay");
    } else {
        noSuggestionsDiv.classList.add("noDisplay");
    }
}

let mobFilter = document.getElementsByClassName("mobFilter");
if (mobFilter.length > 0) {
    mobFilter[0].addEventListener("click", function (e) {
        let mobFilterSec = document.getElementsByClassName("wrapper");


        mobFilterSec[0].classList.toggle("hidden-xs");


    });
}

var disclaimerStrip = document.getElementsByClassName("strip");
function errorStatusCode(flag) {
    let searchSections = [...document.getElementsByClassName("searchDataSec")];
    if(disclaimerStrip.length > 0){
    searchSections.push(document.getElementsByClassName("strip")[0]);
    }
    let errorSection = document.getElementsByClassName("errSec")[0];
    if (flag == true) {
        for (let section of searchSections) {
        ...
        for (let section of searchSections) {
            if (!section.classList.contains("noRes")){
...
var scrollToFilter = document.querySelector(".scrollToFilter");
var  textLink = document.createElement('a');
textLink.innerHTML = "Click here ";
scrollToFilter.textContent += " ";
scrollToFilter.append(textLink);
var aTagAfterText = document.createTextNode("to navigate.");
scrollToFilter.append(aTagAfterText);
var searchJobSec = document.querySelector(".searchJobSec");
textLink.addEventListener('click',function(){
    searchJobSec.scrollIntoView();
})
