...
// this code for hedaer visable---------------------------
var audio = document.querySelectorAll(".startTime")[0];
if (audio) {
    audio.innerHTML = (Math.round(startTime * 100) / 100).toFixed(2);
    document.querySelectorAll(".lineSeparation")[0].innerHTML = "/";
    document.querySelectorAll(".endTime")[0].innerHTML = (Math.round(endTime * 100) / 100).toFixed(2);
}

/* for fixed background image - parallax*/
if (window.innerWidth > 991) {
    if (document.getElementsByClassName("bgCoffeeStory1").length > 0) {
        var secondDiv = document.getElementsByClassName("bgCoffeeStory1")[0];
        var fixedImage1 = document.getElementsByClassName("fixedImage1")[0].getAttribute("src");
        secondDiv.style.backgroundImage = "url('" + fixedImage1 + "')";
        secondDiv.style.backgroundAttachment = "fixed";
    }
    if (document.getElementsByClassName("bgCoffeeStory2").length > 0) {
        var bgCoffeeStory2 = document.getElementsByClassName("bgCoffeeStory2");
        var fixedImage2 = document.getElementsByClassName("fixedImage2");
        for (var i = 0; i < bgCoffeeStory2.length; i++){
            bgCoffeeStory2[i].style.backgroundImage = "url('" + fixedImage2[i].getAttribute("src") + "')";
            bgCoffeeStory2[i].style.backgroundAttachment = "fixed";
        }
    }
    /* for Dynamic Height */
    var heroForegrounds = document.getElementsByClassName("heroForegrounds");
    for (var i = 1; i < heroForegrounds.length; i++) {
        heroForegrounds[i].style.height = "auto"
        heroForegrounds[i].style.position = "relative"
    }
}

if (audio) {
    var audio = document.querySelectorAll(".audioDiv")[0];
    var audioIcon = document.querySelectorAll(".audioIcon")[0];
    var audioIcons = document.querySelectorAll(".audioIcons")[0];

    audioIcon.addEventListener("click", function () {
        var audioIconHover = document.querySelector(".audioIconHover")
        audioIconHover.classList.add("noDisplay")
        if (audioIcons.classList.contains("noDisplay")) {
            audioIcons.classList.remove("noDisplay");
            audioIconHover.classList.add("removeHover");
            audioIcon.classList.remove("soundImg");
            audioIcon.classList.add("closeSoundImg");
        } else {
            audioIcon.classList.remove("audioSpeakerIconFixed");
            audioIcons.classList.remove("audioIconFixed")
            audioIcon.classList.add("soundImg");
            audioIcon.classList.remove("closeSoundImg");
            audioIconHover.classList.remove("removeHover");
            audioIcons.classList.add("noDisplay");

            var audioCtrlAllUp = document.querySelector(".audioCtrlAllUp");
            if (audioCtrlAllUp) {
                audioIconHover.classList.add("removeHover");
                audioIcon.classList.add("noDisplay");
            }
            audio.pause();
            audio.currentTime = 0;
            playIcon.classList.remove("noDisplay");
            pauseIcon.classList.add("noDisplay");
        }
    });

    var playPause = document.querySelectorAll(".playPauseCtrl")[0];
    var playIcon = document.querySelectorAll(".playImg")[0];
    var pauseIcon = document.querySelectorAll(".pauseImg")[0];
    playPause.addEventListener("click", function () {
        if (playPause.length != 0) {
            for (var i = 0; i < playPause.children.length; i++) {
                if (playPause.children[i].classList.contains("noDisplay")) {
                    playPause.children[i].classList.remove("noDisplay")
                } else
                    playPause.children[i].classList.add("noDisplay")
            }
    ...
    var seekBar = document.querySelectorAll(".audioBar")[0];
    audio.addEventListener("timeupdate", function () {
        ...
        var val, str = '';
        this.setAttribute('value', Math.floor((audio.currentTime / audio.duration) * 100));
        val = Math.floor((audio.currentTime / audio.duration) * 100) + '% 100%';
    ...
    audio.addEventListener("ended", function () {
        if (playIcon.classList.contains("noDisplay")) {
            playIcon.classList.remove("noDisplay");
            pauseIcon.classList.add("noDisplay");
        }
    })
    ...
    function showAudioTime() {
        ...
        var endTime = audio.duration - startTime;
        document.querySelectorAll(".startTime")[0].innerHTML = (Math.round(startTime * 100) / 100).toFixed(2);
        document.querySelectorAll(".lineSeparation")[0].innerHTML = "/";
        document.querySelectorAll(".endTime")[0].innerHTML = (Math.round(endTime * 100) / 100).toFixed(2);
    }
...
if (window.innerWidth < 992) {
    ...
    var coffDate = document.getElementsByClassName("coffDate");
    var coffeeScrollDiv = document.getElementsByClassName("coffeeScrollDiv");

    for (var i = 0; i < positioning.length; i++) {
        positioning[i].classList.remove("animm");
        if (coffDate.length > 0) {
            coffDate[i].classList.remove("animm");
        }
    }
    for (var i = 0; i < coffeeScrollDiv.length; i++) {
        coffeeScrollDiv[i].classList.remove("animm");
    }
    var removeMainSlideElem = document.getElementsByClassName("mainSliderElem");
    for (var i = 2; i < removeMainSlideElem.length; i++) {
        removeMainSlideElem[i].classList.remove("mainSliderElem");
    }
    document.addEventListener("DOMContentLoaded", function (event) {
        var removeMainSlideElem = document.getElementsByClassName("noSlider");
        for (var i = 1; i < removeMainSlideElem.length; i++) {
            removeMainSlideElem[i].classList.remove("mainSliderElem");
        }
    });
}
if (saltDate) {
    saltDate.classList.add("marginTop6");
}

var tagItemsCoffee = document.querySelectorAll(".tagItems");
for (var i = 0; i < tagItemsCoffee.length; i++) {
    tagItemsCoffee[i].addEventListener("click", function (event) {
