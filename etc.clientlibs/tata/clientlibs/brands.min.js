...
var verSearchIp1 = document.getElementById("searchVerticals");
var locateBrands1 = document.getElementsByClassName("locateBrands")[0];
verSearchIp1.addEventListener('click', function(event) {
    locateBrands1.scrollIntoView();
});

var over = document.getElementsByClassName("over");
var tabListArray = [];
for (var i = 0; i < over.length; i++) {
    tabListArray.push(over[i]);
    over[i].addEventListener("click", function() {
        if (this.querySelector(".brandtablinks.activeLink"))
            return;
        var el = this;
        var tabNo = tabListArray.indexOf(el);
        var loadM = document.getElementsByClassName("flexx");
        for (var i = 0; i < loadM.length; i++) {
            loadM[i].style.display = "block";
        }
        var loadMm = document.getElementsByClassName("brandMobile");
        for (i = 0; i < loadMm.length; i++) {
            loadMm[i].style.display = "block";
        }
        var loadMmm = document.getElementsByClassName("loadF");
        for (i = 0; i < loadMmm.length; i++) {
            loadMmm[i].style.display = "none";
        }

        var loadMoreD = document.querySelectorAll(".flexD");

        if (loadMoreD.length != 0) {
            for (var i = 0; i < loadMoreD.length; i++) {
                loadMoreD[i].children[0].children[0].innerHTML = "Load More"
            }
        }
        openbrands(el, tabNo);
    });
...
//this method use for brands filter
function openbrands(evt, cityNo) {
    ...
    tablinks = document.getElementsByClassName("brandtablinks");
    for (i = 0; i < tablinks.length; i++) {
        tablinks[i].className = tablinks[i].className.replace("active", "");
    }

    var brandtabimage = document.querySelectorAll(".brandtabimage")[0];

    for (var i = 0; i < brandtabimage.children.length; i++) {
        if (i == cityNo) {
            brandtabimage.children[i].children[2].style.opacity = "#307FE2";
            brandtabimage.children[i].children[2].classList.add("activeLink");
            for (var k = 0; k < brandtabimage.children[i].children.length; k++) {
                var child = brandtabimage.children[i].children[k];
                if (child.classList.contains("centered") && child.classList.contains("noDisplay")) {
                    child.classList.remove("noDisplay")
                } else if (child.classList.contains("centered")) {
                    child.classList.add("noDisplay")
                }
            }
        } else {
            brandtabimage.children[i].children[2].style.color = "#000";
            brandtabimage.children[i].children[2].classList.remove("activeLink");
            for (var k = 0; k < brandtabimage.children[i].children.length; k++) {
                var child = brandtabimage.children[i].children[k];
                if (child.classList.contains("centered") && child.classList.contains("active")) {
                    child.classList.add("noDisplay")
                } else if (child.classList.contains("centered")) {
                    child.classList.remove("noDisplay")
                }
    ...
    var tabContents = document.querySelectorAll(".tabContents")[0];
...
//this code for Non Desktop check  load more functinality on multi image carousel
var loadMoreC = document.getElementsByClassName("loadMoreC");
if (loadMoreC.length != 0) {
    for (var i = 0; i < loadMoreC.length; i++) {
        loadMoreC[i].addEventListener("click", loadMoreContent);
    }

    function loadMoreContent() {
        this.style.display = "none";
        (this.parentNode.nextElementSibling).style.display = "block";
        if (this.parentNode.nextElementSibling.nextElementSibling) {
            (this.parentNode.nextElementSibling.nextElementSibling).style.display = "block"
        }
    }
}

/*- Dropdown for brands tab in mobile view */
var brandsTab = document.querySelectorAll(".brandsTab")[0].children[0].children[0].children[0];
var brandNames = [];
var brandImages = [];
var brandImagesActive = [];
var path = "/content/dam/tata/images/brands/desktop/tabs";
for (var i = 0; i < brandsTab.children.length; i++) {
    var src = brandsTab.children[i].children[0].src;
    var index = src.lastIndexOf("/");
    brandImages.push(path + src.substring(index));
    var src1 = brandsTab.children[i].children[1].src;
    var index1 = src1.lastIndexOf("/");
    brandImagesActive.push(path + src1.substring(index1));
    brandNames.push(brandsTab.children[i].children[2].innerHTML);
}
...
for (var i = 0; i < brandNames.length; i++) {
    ...
    var img = document.createElement('img');
    img.setAttribute('src', brandImages[i]);
    img.setAttribute('alt', brandNames[i]);
    img.setAttribute('class', "brandimg active");
    var imgActive = document.createElement('img');
    imgActive.setAttribute('src', brandImagesActive[i]);
    imgActive.setAttribute('alt', brandNames[i]);
    imgActive.setAttribute('class', "brandimg noDisplay");
    var div = document.createElement('div');
    div.setAttribute('class', 'listBrand');
    div.innerHTML = brandNames[i];
    parentDiv.appendChild(img);
    parentDiv.appendChild(imgActive);
...
selected.children[0].src = brandImagesActive[0];
selected.children[0].alt = brandNames[0];
selected.children[1].innerHTML = brandNames[0];
selected.children[1].classList.add("selectedItem");
list.children[0].children[0].classList.add("noDisplay");
list.children[0].children[1].classList.remove("noDisplay");
list.children[0].children[2].classList.add("selectedItem");
selIndex = 0;
var tabContents = document.querySelectorAll(".tabContents")[0];
var prevIndex = 1;
var dropdwn = document.querySelectorAll(".dropdwn")[0];
var arrow = document.querySelectorAll(".arrowMob")[0];
var flag = false;
//this code for mobile only use
dropdwn.addEventListener("click", function() {

    flag = false;
    dropdwn.classList.add("boxShadow")
    ...
    if (list.classList.contains("noDisplay")) {
        ...
        dropdwn.classList.remove("boxShadow")
    }

    if (list.children.length !== 0) {
        var len = list.children.length;
        for (var i = 0; i < len; i++) {
            list.children[i].addEventListener("click", getLists);
        }
    }
})
...
function getLists() {
    ...
    arrow.classList.add("downarrow");
    dropdwn.classList.remove("boxShadow")
    var prev = document.getElementsByClassName("selectedItem")[1];
    var presentItem = el.children[2]
    if (presentItem != prev) {
        for (var j = 0; j < list.children.length; j++) {
            if (list.children[j].children[2].innerHTML == prev.innerHTML) {
                prevIndex = j;
            }
        }
        ...
        var loadMm = document.getElementsByClassName("brandMobile");
        for (i = 0; i < loadMm.length; i++) {
            loadMm[i].style.display = "none";
        }
        var loadMmm = document.getElementsByClassName("loadF");
        for (i = 0; i < loadMmm.length; i++) {
            loadMmm[i].style.display = "none";
        }
        var loadMoreXs = document.querySelectorAll(".loadMoreXs");
        if (loadMoreXs.length != 0) {
            for (var i = 0; i < loadMoreXs.length; i++) {
                loadMoreXs[i].children[0].children[0].innerHTML = "Load More"
            }
        }

        prev.classList.remove("selectedItem");
        prev.previousSibling.classList.remove("active");
        prev.previousSibling.classList.add("noDisplay");
        prev.previousSibling.previousSibling.classList.remove("noDisplay");
        prev.previousSibling.previousSibling.classList.add("active");
        document.getElementsByClassName("parentdiv")[selIndex].lastChild.classList.add("selectedItem");
        document.getElementsByClassName("parentdiv")[selIndex].firstChild.classList.add("noDisplay");
        document.getElementsByClassName("parentdiv")[selIndex].firstChild.classList.remove("active");
        document.getElementsByClassName("parentdiv")[selIndex].firstChild.nextSibling.classList.add("active");
        document.getElementsByClassName("parentdiv")[selIndex].firstChild.nextSibling.classList.remove("noDisplay");
        document.getElementsByClassName("selectedItem")[0].innerHTML = document.getElementsByClassName("parentdiv")[selIndex].lastChild.innerHTML;
        selected.children[0].src = document.getElementsByClassName("parentdiv")[selIndex].firstChild.nextSibling.src;
        document.getElementsByClassName("activeContent")[0].classList.add("mobNoDisp");
        document.getElementsByClassName("tabcontent")[selIndex].classList.remove("mobNoDisp");
        document.getElementsByClassName("tabcontent")[prevIndex].classList.add("mobNoDisp");
    }
}


//this code for close all list tab in brands tab
function closeList() {
    if (!list.classList.contains("noDisplay") && flag) {
        list.classList.add("noDisplay")
        arrow.classList.remove("uparrow");
        arrow.classList.add("downarrow");
        dropdwn.classList.remove("boxShadow")
