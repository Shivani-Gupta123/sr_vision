...
if(document.getElementsByClassName("activeMenu").length == 0){
	navActiveItem[3].classList.add("activeMenu");
}
...
var tooltip = document.querySelectorAll(".abtUsTooltip");
var tooltiptext = document.querySelectorAll(".tooltiptext");
var readMoreIcon = document.getElementsByClassName("readMoreIcon");
var toolTipFlag = false;

//showing tooltip as block element for desktop and ipad
if (window.innerWidth > 575) {
    var toolview = document.querySelectorAll(".abtUsTooltip .tooltiptext");
    for (var i = 0; i < toolview.length; i++) {
        toolview[i].classList.remove("invisible");
        toolview[i].style.display = "block";
    }
}

//removing animation as one touch scroll for mobile devices
if (window.innerWidth < 575) {
    for (var i = 0; i < readMoreIcon.length; i++) {
        readMoreIcon[i].addEventListener("click", function () {
            (this.nextElementSibling.children[1]).classList.toggle("invisible");
            if (!this.nextElementSibling.children[1].classList.contains("invisible"))
                toolTipFlag = true;
        });
    }
    var sectionContent = document.getElementsByClassName("sectionContent");
    var abtUsTooltip = document.getElementsByClassName("abtUsTooltip");
    for (var i = 0; i < sectionContent.length; i++) {
        sectionContent[i].classList.remove("animm");
    }
    for (var i = 0; i < abtUsTooltip.length; i++) {
        abtUsTooltip[i].classList.remove("animm");
    }
    //closing the tooltip on outside click
    function closeToolTip() {
        var temp = document.getElementsByClassName("tooltiptext");
        if (!toolTipFlag) {
            for (var i = 0; i < temp.length; i++) {
                temp[i].classList.add("invisible");
            }
        }
        toolTipFlag = false;
    }
    document.addEventListener("click", closeToolTip, false);
}


// //parallX adjustment for about-us page code 
var parallX = document.querySelector('.parallaxControls');
            var parallaxControls = document.querySelector('.parallaxControls');
 let parallXFooter = document.getElementsByClassName("about-us-parallax")[0];
 var parallXFooterHeight=parallXFooter.scrollHeight;
if (parallXFooter) {
    window.addEventListener('scroll', function () {
        if ((window.innerHeight + window.scrollY) >= document.body.offsetHeight - parallXFooterHeight+100) {
