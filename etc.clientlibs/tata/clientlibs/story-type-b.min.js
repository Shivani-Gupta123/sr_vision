...
var loadMoreBtn1 = document.getElementsByClassName("hockeyButton");
var loadLessButton = document.getElementsByClassName("buttonLess");
if (loadMoreBtn1.length != 0) {
    loadMoreBtn1[0].addEventListener("click", function () {
        var viewHide = document.querySelectorAll(".viewHide");
        for (var i = 0; i < viewHide.length; i++) {
            viewHide[i].style.display = "block";
        }
        loadMoreBtn1[0].style.display = "none";
        loadLessButton[0].style.display = "block";
		 shareIconHandler();
    });
	
}

if (loadLessButton.length != 0) {
    loadLessButton[0].addEventListener("click", function () {
        var viewHide = document.querySelectorAll(".viewHide");
        var elem = document.getElementsByClassName("paraDiv")[0];
        var viewMore = elem.getElementsByClassName("showViewMore")[0];
        var elemView = [...viewMore.querySelectorAll(".saltPara, .hockeyImg")].pop();

        elemView.scrollIntoView();

        for (var i = 0; i < viewHide.length; i++) {
            viewHide[i].style.display = "none";
        }
        loadLessButton[0].style.display = "none";
        loadMoreBtn1[0].style.display = "block";
		 shareIconHandler();
...
if (window.innerWidth < 992) {
    var animm = document.getElementsByClassName("animm");
    var positioning = document.getElementsByClassName("positioning");

    for (var i = 0; i < positioning.length; i++) {
        positioning[i].classList.remove("animm");
    }
}

var tagItemshockey = document.querySelectorAll(".tagItems");
for (var i = 0; i < tagItemshockey.length; i++) {
    tagItemshockey[i].addEventListener("click", function (event) {
        var hashVal = this.children[0].innerHTML;
        localStorage.searchBar = true;
        if (hashVal !== "") {
            var searchBoxIcon = document.getElementsByClassName("searchBoxIcon")[0];
            localStorage.hashValue = hashVal;
            searchBoxIcon.click();
            searchBtnClicked = true;
            searchDetails(event, 1);
        }
...
var videoElement = document.getElementsByClassName("videoContainerdiv");
var bannerVideo = document.getElementsByClassName("bannerVideoContdiv");
var src, posterImage;
...
if (videoElement.length !== 0) {
    count = videoElement.length;
    videoElement = [...videoElement];
    for (let i = 0; i < videoElement.length;i +=1)
    {
        
        videoElement[i].id = "divVideo" + i + 'C';
        let videoDiv = videoElement[i].dataset;
        src = videoDiv.href;
        posterImage = videoDiv.image;
        let playerInstance = jwplayer(videoElement[i].id);
    playerInstance.setup({
        "skin": {
            "name": "myskin"
        },
    ...
    let markers = [25,50,75]; //To decide the percentage which want to track
    let playersMarkers = [];
    ...
    playerInstance.on('time', function(e){
        var percentPlayed = Math.floor(100 * e.position / e.duration);
        let percentPlayedupper = Math.ceil( 100 * e.position / e.duration );
        var playerMarkerIndex = findObjectIndexById(playersMarkers,'id',this.id); 
    ...
    playerInstance.on('play',function(e){
        var playerMarkerIndex = findObjectIndexById(playersMarkers,'id',this.id); 
        if (playersMarkers[playerMarkerIndex].paused) {
            var pct =  Math.floor(100 * playerInstance.getPosition() / playerInstance.getDuration());
            addToDataLayerWithPct(src,"Play",pct);
            //make current video paused status as false
            playersMarkers[playerMarkerIndex].paused = false;
        }
    });
    ...
    playerInstance.on('pause',function(e){
        var pct =  Math.floor(100 * playerInstance.getPosition() / playerInstance.getDuration());
        addToDataLayerWithPct(src,"Pause",pct);
        //to make current video status paused
        var playerMarkerIndex = findObjectIndexById(playersMarkers,'id',this.id); 
        playersMarkers[playerMarkerIndex].paused = true;
    });
...
//added code for removing mainSliderElem class from neverRest section 
var neverRestfind = document.querySelectorAll("[class*=neverRest]")
if (neverRestfind.length != 0) {
    for (var i = 0; i < neverRestfind.length; i++) {
        neverRestfind[i].classList.remove("mainSliderElem");
    }
}

var storyAudiobar = document.querySelector(".storyAudiobar");

if(storyAudiobar){
var audioContainer = document.querySelector(".audioContainer");
var music = document.querySelector(".audioSrcDiv audio");
var play = document.querySelector(".play");
var pause = document.querySelector(".pause");
var audio_controls = document.querySelector(".audio_controls");
var play_pause_btn = document.querySelector(".play-pause-btn");
var play_button = document.querySelector(".play");
var pause_button = document.querySelector(".pause");
var play_pause_toggle = document.querySelector(".play-pause-toggle");
var  progress_controls = document.querySelector(".progress-controls");
var progress_area = document.querySelector(".progress-area");
...
var volume_control = document.querySelector(".volume-control");
var volume_btn = document.querySelector(".volume-btn");
var full_volume = document.querySelector(".full-volume");
var half_volume = document.querySelector(".half-volume");
var muted = document.querySelector(".muted");
var volume_panel = document.querySelector(".volume-panel");
var volume_input_div = document.querySelector(".volume-input-div");
var volumeSlider = document.querySelector("#volumeSlider");
var playback_speed = document.querySelector(".playback_speed");
var playback_speed_btn = document.querySelector(".playback-speed-btn");
var audio_playbackspeed = document.querySelector(".audio-playbackspeed");
var speedRate_list = document.querySelector(".speedRate_list");
var speedRatelistItems = document.querySelectorAll(".speedRate_list li");
var playSpeed = document.querySelector("#playSpeed");
var playpausetooltip = document.querySelector("#playpausetooltip");
var playbackspeedtooltip = document.querySelector("#playbackspeedtooltip");
//Function to add audio events in datalayer for google analytics  
function addAudioToDataLayerWithPct(src,action,pct){
    if(isNaN(pct)){
        pct = 0;
    }
    dataLayer.push({
...
var play_src = play_pause_btn.getElementsByTagName('img')[0].getAttribute('src');
var playOrg_src = play_pause_btn.getElementsByTagName('img')[1].getAttribute('src');
var pause_src = play_pause_btn.getElementsByTagName('img')[2].getAttribute('src');
var pauseOrg_src = play_pause_btn.getElementsByTagName('img')[3].getAttribute('src');
...
play_pause_btn.addEventListener('click',(e)=>{
    if(music.paused || music.currentTime <= 0){
        ...
        playpausetooltip.innerHTML = "Pause";
        addAudioToDataLayerWithPct(music.src,"Play",Math.floor(100 * music.currentTime/ music.duration));
        playersMarkers[0].paused = false;
    }
    else{
        music.pause();
        play_button.style.display = "block";
        pause_button.style.display = "none";
        playpausetooltip.innerHTML = "Play";
        addAudioToDataLayerWithPct(music.src,"Pause",Math.floor(100 * music.currentTime/ music.duration));
        playersMarkers[0].paused = true;
    }
    if(window.innerWidth < 991){
        if(play){
            play_pause_btn.getElementsByTagName('img')[0].src = playOrg_src;
            setTimeout(function () {
                play_pause_btn.getElementsByTagName('img')[0].src = play_src;
            }, 200);
        }
        if(pause){
            play_pause_btn.getElementsByTagName('img')[2].src = pauseOrg_src;
            setTimeout(function () {
                play_pause_btn.getElementsByTagName('img')[2].src = pause_src;
            }, 200);
        }
    }
});
...
window.addEventListener("DOMContentLoaded",showTotalTime);
window.addEventListener("load",showTotalTime);
music.addEventListener("loadedData",showTotalTime);

//current audio duration
music.addEventListener("timeupdate",(e)=>{
    var currentAudioTime = e.target.currentTime;
    var currentMin = Math.floor(currentAudioTime / 60);
    var currentSec = Math.floor(currentAudioTime % 60);
    //if seconds are less than 10 then add 0 at the begning
    currentSec < 10 ? currentSec = "0"+currentSec : currentSec;
    current.innerHTML = `${currentMin} : ${currentSec}`; 
    //progress bar width change as well time
    var audioduration = e.target.duration;
    var progressWidth = (currentAudioTime / audioduration) * 100;
    // progress_bar.style.width = `${progressWidth}%`;  
    progress_bar.value = progressWidth;
    progress_bar.style.background = `linear-gradient(to right, rgb(255, 174, 0) 0%, rgb(255, 174, 0) ${progressWidth}%, #BEBEBE ${progressWidth}%, #BEBEBE 100%)`;

    let percentPlayed = Math.floor( 100 * currentAudioTime/ audioduration );
    let percentPlayedupper = Math.ceil( 100 * currentAudioTime / audioduration );         
    if (markers.indexOf(percentPlayed)>-1 && playersMarkers[0].markers.indexOf(percentPlayed)==-1) {
        playersMarkers[0].markers.push(percentPlayed);
        addAudioToDataLayerWithPct(music.src,"Progress",percentPlayed);
    }
    else if (markers.indexOf(percentPlayedupper)>-1 && playersMarkers[0].markers.indexOf(percentPlayedupper)==-1) {
        playersMarkers[0].markers.push(percentPlayedupper);
        addAudioToDataLayerWithPct(music.src,"Progress",percentPlayedupper);
    }
});  
//draggable progress bar 
//function draggProgessbar(e){
//    var progressWidthValue = progress_area.clientWidth;
//    progress_bar.style.width = `${e.offsetX}px`;
//    music.currentTime = (e.offsetX / progressWidthValue) *music.duration;
//} 
//progress_area.addEventListener('mousedown',(e)=>{
//    progress_area.addEventListener('mousemove', draggProgessbar);
//});
//progress_area.addEventListener('mouseup',(e)=>{
//    progress_area.removeEventListener('mousemove', draggProgessbar);
//});
   // draggable progress bar for touch devices
//   progress_area.addEventListener('touchstart',(e)=>{
//    progress_area.addEventListener('touchmove', draggProgessbar);
//});
//progress_area.addEventListener('touchend',(e)=>{
//    progress_area.removeEventListener('touchmove', draggProgessbar);
//});
//update playing audio current time on according to the progress bar width
//progress_area.addEventListener('click',(e)=>{
//    var audioduration = music.duration;
//    var progressWidthValue = progress_area.clientWidth;
//    var clickWidth = e.offsetX;
//    var currentTime = (clickWidth / progressWidthValue) * audioduration
//    music.currentTime = currentTime;
//    var pct = Math.floor(100 * currentTime / audioduration);
//    addAudioToDataLayerWithPct(music.src,"Seek",pct);
//});

/*Code for progress bar */
const progress_min = progress_bar.min;
const progress_max = progress_bar.max;
const progress_value = progress_bar.value;

progress_bar.style.background = `linear-gradient(to right, rgb(255, 174, 0) 0%, rgb(255, 174, 0) ${(progress_value-progress_min)/(progress_max-progress_min)*100}%, #BEBEBE ${(progress_value-progress_min)/(progress_max-progress_min)*100}%, #BEBEBE 100%)`;
progress_bar.addEventListener('input',(e)=>{
	/*Code to update current time */
    var currentAudioTime = music.currentTime;
    var currentMin = Math.floor(currentAudioTime / 60);
    var currentSec = Math.floor(currentAudioTime % 60);
    //if seconds are less than 10 then add 0 at the begning
    currentSec < 10 ? currentSec = "0"+currentSec : currentSec;
    ...
    var progress_min = progress_bar.min;
    var progress_max = progress_bar.max;
    var progress_value = progress_bar.value;
    music.currentTime = e.target.value / 100 * music.duration ;
    e.target.style.background = `linear-gradient(to right, rgb(255, 174, 0) 0%, rgb(255, 174, 0) ${(progress_value-progress_min)/(progress_max-progress_min)*100}%, #BEBEBE ${(progress_value-progress_min)/(progress_max-progress_min)*100}%, #BEBEBE 100%)`;
    //addAudioToDataLayerWithPct(music.src,"Seek", Math.floor(progress_value));
});

/*Code to capture seek events in GA */
progress_bar.addEventListener('mouseup',(e)=>{
    var progress_value = progress_bar.value;
    addAudioToDataLayerWithPct(music.src,"Seek", Math.floor(progress_value));
});
progress_bar.addEventListener('touchend',(e)=>{
    var progress_value = progress_bar.value;
    addAudioToDataLayerWithPct(music.src,"Seek", Math.floor(progress_value));
});

//change play icon after ended audio 
music.addEventListener('ended', ()=>{
    play_button.style.display = "block";
    pause_button.style.display = "none";
    playersMarkers[0].markers=[];
    addAudioToDataLayerWithPct(music.src,"Complete",100);
    //make video paused status true
    playersMarkers[0].paused = true;
});

//playback speed
var img_src = playback_speed_btn.getElementsByTagName('img')[0].getAttribute('src');
var hoverImg_src = playback_speed_btn.getElementsByTagName('img')[1].getAttribute('src');
var orignalImg_src = img_src;
playback_speed_btn.addEventListener('click', ()=>{
    speedRate_list.classList.toggle('active');
    if(speedRate_list.classList.contains('active')){
        playback_speed_btn.getElementsByTagName('img')[0].src = hoverImg_src;
    }else{
        playback_speed_btn.getElementsByTagName('img')[0].src = orignalImg_src;
    }
});

document.addEventListener('click', function(e) {
    if (!playback_speed_btn.contains(e.target)) {
        speedRate_list.classList.remove('active');
        if(!speedRate_list.classList.contains('active')){
            playback_speed_btn.getElementsByTagName('img')[0].src = orignalImg_src;
        }
    }
    if(speedRate_list.classList.contains('active')){
        playbackspeedtooltip.style.display="none";
        volume_panel.style.display="none";
    }else{
        playbackspeedtooltip.style.display="block";
        volume_panel.removeAttribute("style");
    }
});

speedRatelistItems.forEach((event) =>{
...
volumeSlider.style.background = `linear-gradient(to right, rgb(255, 174, 0) 0%, rgb(255, 174, 0) ${(value-min)/(max-min)*100}%, #BEBEBE ${(value-min)/(max-min)*100}%, #BEBEBE 100%)`;

volumeSlider.oninput = function() {
    var value = (this.value-this.min)/(this.max-this.min)*100
    this.style.background = 'linear-gradient(to right, rgb(255, 174, 0) 0%, rgb(255, 174, 0) ' + value + '%, #BEBEBE ' + value + '%, #BEBEBE 100%)';
};
...
music.addEventListener('volumechange', ()=>{
    ...
    if(music.muted || music.volume === 0){
        ...
        full_volume.style.display ='none';
        half_volume.style.display = 'none';
    }
    else if(music.volume >= 0 && music.volume <= 0.75){
        muted.style.display = "none";
        full_volume.style.display ='none';
        half_volume.style.display = 'block'
    }
    else{
        full_volume.style.display ='block';
        muted.style.display = "none";
        half_volume.style.display = 'none'
    }
});
volumeSlider.addEventListener('input',(e)=>{
    ...
    currentValue = e.target.value*100;
});
var fullVolImg_src = volume_btn.getElementsByTagName('img')[0].getAttribute('src');
var fullVolImgOrg_src = volume_btn.getElementsByTagName('img')[1].getAttribute('src');
var halfVolImg_src = volume_btn.getElementsByTagName('img')[2].getAttribute('src');
var halfVolImgOrg_src = volume_btn.getElementsByTagName('img')[3].getAttribute('src');
var mutedImg_src = volume_btn.getElementsByTagName('img')[4].getAttribute('src');
var mutedImgOrg_src = volume_btn.getElementsByTagName('img')[5].getAttribute('src');
volume_btn.addEventListener('click',()=>{
    music.muted = !music.muted;
    if(!music.muted){
        volumeSlider.style.background = 'linear-gradient(to right, rgb(255, 174, 0) 0%, rgb(255, 174, 0) ' + currentValue + '%, #BEBEBE ' + currentValue + '%, #BEBEBE 100%)';
    }else{
        volumeSlider.style.background = 'linear-gradient(to right, rgb(255, 174, 0) 0%, rgb(255, 174, 0) ' + value + '%, #BEBEBE ' +value + '%, #BEBEBE 100%)';
    }  
    // change icon color after click
    if(window.innerWidth < 991){
        if(full_volume){
            volume_btn.getElementsByTagName('img')[0].src = fullVolImgOrg_src;
            setTimeout(function () {
                volume_btn.getElementsByTagName('img')[0].src = fullVolImg_src;
            }, 200);
        }
        if(muted){
            volume_btn.getElementsByTagName('img')[4].src = mutedImgOrg_src;
            setTimeout(function () {
                volume_btn.getElementsByTagName('img')[4].src = mutedImg_src;
            }, 200);
        }
    }  
});
/*Code to change icons on hover to orange icons */
if(window.innerWidth > 991){
play_pause_btn.addEventListener('mouseover',(e)=>{
        play_pause_btn.getElementsByTagName('img')[0].src = playOrg_src;
        play_pause_btn.getElementsByTagName('img')[2].src = pauseOrg_src;
        // play_button.src = "../images/icons/Audio/play_orange.svg";
        // pause_button.src = "../images/icons/Audio/pause_orange.svg";
    });
        play_pause_btn.addEventListener('mouseout',(e)=>{
        play_pause_btn.getElementsByTagName('img')[0].src = play_src;
        play_pause_btn.getElementsByTagName('img')[2].src = pause_src
        // play_button.src = "../images/icons/Audio/play.svg";
        // pause_button.src = "../images/icons/Audio/pause.svg";
        });
    // playback_speed_btn.addEventListener('mouseover', ()=>{
    //     playSpeed.src = "../images/icons/Audio/playback_orange.svg";
    // });
    // playback_speed_btn.addEventListener('mouseout', ()=>{
    //     playSpeed.src = "../images/icons/Audio/playback.svg";
    // });
    
    // var full_volume = document.querySelector(".full-volume");
    // var half_volume = document.querySelector(".half-volume");
    // var muted = document.querySelector(".muted");
        volume_btn.addEventListener('mouseover',(e)=>{
        volume_btn.getElementsByTagName('img')[0].src = fullVolImgOrg_src;
        volume_btn.getElementsByTagName('img')[2].src = halfVolImgOrg_src;
        volume_btn.getElementsByTagName('img')[4].src = mutedImgOrg_src;
        // full_volume.src = "../images/icons/Audio/volume_full_orange.svg";
        // half_volume.src = "../images/icons/Audio/volume_half_orange.svg";
        // muted.src = "../images/icons/Audio/volume_mute_orange.svg"; 
        })
        volume_btn.addEventListener('mouseout',(e)=>{
        volume_btn.getElementsByTagName('img')[0].src = fullVolImg_src;
        volume_btn.getElementsByTagName('img')[2].src = halfVolImg_src;
        volume_btn.getElementsByTagName('img')[4].src = mutedImg_src; 
        // full_volume.src = "../images/icons/Audio/volume_full.svg";
        // half_volume.src = "../images/icons/Audio/volume_half.svg";
        // muted.src = "../images/icons/Audio/volume_mute.svg";
        });
    }
}
let dropdowns = (document.getElementsByClassName("dropdownElement"));

for (let element of dropdowns) {
    let children = [].slice.call(element.children);
    let btn = children.filter((child) => {
        return child.classList.contains("btnDropdown")
    })[0];
    let text = children.filter((child) => {
        return child.classList.contains("dropdownText");
    })[0];
    text.style.height = "0px";
    btn.addEventListener("click", function () { showToggle(text,btn) });
    btn.addEventListener("touchstart", (e)=>{
        btn.classList.toggle("arrowBackgroundHover");
        btn.children[0].children[0].classList.toggle("arrowHover");
    })
...
function showToggle(text,btn) {
    ...
    btn.children[0].classList.toggle('rotatedArrow')
...
//this code for desktop check load more button and load more functinality 
var loadMoreD = document.querySelectorAll(".flexD > .buttonMore");
if (loadMoreD.length != 0) {
    for (i = 0; i < loadMoreD.length; i++) {
        loadMoreD[i].addEventListener("click", function () {
            var e2 = this;
            var loadMoreTop = e2.parentElement.parentElement.parentElement;
            if (e2.parentElement.parentElement.parentElement.children[1].children[0].children[0].children[0].innerHTML == "Load Less") {
                e2.parentElement.parentElement.parentElement.children[0].children[4].style.display = "none";
                e2.parentElement.parentElement.parentElement.children[1].children[0].children[0].children[0].innerHTML = "Load More";
                if (loadMoreTop) {
                    loadMoreTop.scrollIntoView();
                }
            } else {
                e2.parentElement.parentElement.parentElement.children[0].children[4].style.display = "block";
                e2.parentElement.parentElement.parentElement.children[1].children[0].children[0].children[0].innerHTML = "Load Less";
            }
        });
    }
}

//this code for Non Desktop check load more button and load more functinality 
var loadMoreXs = document.querySelectorAll(".loadMoreXs > .buttonMore");
if (loadMoreXs.length != 0) {
    for (var i = 0; i < loadMoreXs.length; i++) {
        loadMoreXs[i].addEventListener("click", function () {
            var e1 = this;
            if (e1.parentElement.previousElementSibling)
                (e1.parentElement.previousElementSibling).style.display = "block";

            var loadMoreTopXs = e1.parentElement.parentElement.parentElement;
            var parent = e1.parentElement.parentElement.parentElement.children[0];
            var brandsDiv = parent.getElementsByClassName("removeSm");
            if (e1.parentElement.children[0].children[0].innerHTML == "Load Less") {
                ...
                for (var i = 0; i < parent.children.length; i++) {
                    if (i === 0) {
                        ...
                        if (loadMoreTopXs) {
                            loadMoreTopXs.scrollIntoView();
                        }
                    }
                }
                e1.parentElement.children[0].children[0].innerHTML = "Load More";
            } else {
                for (var i = 0; i < brandsDiv.length; i++) {
                    if (brandsDiv[i].parentNode.classList.contains("flexx")) {
                        brandsDiv[i].parentNode.style.display = "block";
                    }
                }
                e1.parentElement.children[0].children[0].innerHTML = "Load Less";
            }
        });
    }
}
//Load videos on about us page for all folds
function AboutUsVideoLoad() {
    let aboutUsVideos = document.getElementsByClassName("AboutUsBannerVideo");
    for (var i = 0; i < aboutUsVideos.length; i++) {
        if (window.innerWidth > 991) {
            if (aboutUsVideos[i].hasAttribute("desktopVideo")) {
                aboutUsVideos[i].children[0].setAttribute("src", aboutUsVideos[i].getAttribute("desktopVideo"));
                aboutUsVideos[i].load();
                playPauseAboutUsVideo();
                aboutUsVideos[i].parentElement.querySelector(".img-responsive").classList.add("noDisplay");
            }
        } else if (window.innerWidth > 575) {
            if (aboutUsVideos[i].hasAttribute("tabVideo")) {
                aboutUsVideos[i].children[0].setAttribute("src", aboutUsVideos[i].getAttribute("tabVideo"));
                aboutUsVideos[i].load();
                playPauseAboutUsVideo();
                aboutUsVideos[i].parentElement.querySelector(".img-responsive").classList.add("noDisplay");
            }
        } else {
            if (aboutUsVideos[i].hasAttribute("mobileVideo")) {
                aboutUsVideos[i].children[0].setAttribute("src", aboutUsVideos[i].getAttribute("mobileVideo"));
                aboutUsVideos[i].load();
                playPauseAboutUsVideo();
                aboutUsVideos[i].parentElement.querySelector(".img-responsive").classList.add("noDisplay");
            }
...
//Play about us videos on folds based on current scroll position
function playPauseAboutUsVideo() {
    let bottomHalfHeight = (window.innerHeight*2)/10;
    let aboutUsVideos = document.getElementsByClassName("AboutUsBannerVideo");
    for (var i = 0; i < aboutUsVideos.length; i++) {
        if (((window.innerWidth > 991) && (aboutUsVideos[i].hasAttribute("desktopVideo")))
            || ((window.innerWidth < 991 && window.innerWidth > 575) && (aboutUsVideos[i].hasAttribute("desktopVideo")))
            || ((window.innerWidth < 575) && (aboutUsVideos[i].hasAttribute("desktopVideo")))
        ) {
            if ((aboutUsVideos[i].getBoundingClientRect().top > bottomHalfHeight || aboutUsVideos[i].getBoundingClientRect().top < (-1*bottomHalfHeight)) && aboutUsVideos[i].paused === false) {
                aboutUsVideos[i].pause();
            }
            else if (aboutUsVideos[i].getBoundingClientRect().top <= bottomHalfHeight && aboutUsVideos[i].getBoundingClientRect().top > (-1*bottomHalfHeight) && aboutUsVideos[i].paused === true) {
                aboutUsVideos[i].play();
            }
        }
    }
}

document.addEventListener("scroll", playPauseAboutUsVideo);
window.addEventListener("blur", function(){
    let aboutUsVideos = document.getElementsByClassName("AboutUsBannerVideo");
    for (var i = 0; i < aboutUsVideos.length; i++) {
        if (((window.innerWidth > 991) && (aboutUsVideos[i].hasAttribute("desktopVideo")))
...
window.addEventListener("focus", playPauseAboutUsVideo)
//applying background image to section neverest stories
var storyContainer = document.querySelectorAll(".xOverflowHidden")[0];
if (document.querySelectorAll(".brandsTab>img")[0]) {
    storyContainer.previousElementSibling.style.height = storyContainer.clientHeight + "px";
    storyContainer.previousElementSibling.style.width = "100%";
}
var modal = document.getElementsByClassName("myModal");
var imageArray = document.getElementsByClassName('DownlaodPreview');
...
var downloadArray=document.getElementsByClassName('downloadImage');

for(let i=0;i<downloadArray.length;i++)
{
    downloadArray[i].addEventListener('click',function(){

        if(window.innerWidth > 991)
        { 
            if(downloadArray[i].hasAttribute('desktopImage')){
                let filepath=downloadArray[i].getAttribute('desktopImage');
                downloadArray[i].setAttribute('href', filepath);
            }   
        }
        else if(window.innerWidth > 575){
            if(downloadArray[i].hasAttribute('tabImage')){
                let filepath=downloadArray[i].getAttribute('tabImage');
                downloadArray[i].setAttribute('href', filepath);
            }   
        }
        else
        {
            if(downloadArray[i].hasAttribute('mobileImage')){
                let filepath=downloadArray[i].getAttribute('mobileImage');
                downloadArray[i].setAttribute('href', filepath);
            }   
        }
    })  
...
var twitter = document.querySelectorAll(".twitter > .twitterAnchor");
var linkedin = document.querySelectorAll(".linkedin > .linkedinAnchor");
var hangout = document.querySelectorAll(".hangout > .hangoutAnchor");
var mailto = document.querySelectorAll(".mailto > .mailtoAnchor");


for (var i = 0; i < fb.length; i++) {
    if (fb || twitter || linkedin || hangout || mailto) {
        if (fb[i]) {
            fb[i].addEventListener("mouseenter", chngImgColorfb);
        ...
        // if (hangout[i]) {
        //     hangout[i].addEventListener("mouseenter", chngImgColorhangout);

        //     function chngImgColorhangout() {
        //         this.style.backgroundImage = "url(/etc.clientlibs/tata/clientlibs/assets/resources/img/common/shareOptions/color/G+Yellow.svg)";
        //     }
        //     hangout[i].addEventListener("mouseleave", chngImghangout);

        //     function chngImghangout() {
        //         this.style.backgroundImage = "url(/etc.clientlibs/tata/clientlibs/assets/resources/img/common/shareOptions/G+Yellow.svg)";
        //     }
        // }
        if (mailto[i]) {
            mailto[i].addEventListener("mouseenter", chngImgColormailto);
        ...
        fb[i].setAttribute("href", "https://www.facebook.com/sharer/sharer.php?u=" + currentUrl);
        twitter[i].setAttribute("href", "https://twitter.com/share?url=" + currentUrl);
        linkedin[i].setAttribute("href", "http://www.linkedin.com/shareArticle?mini=true&url=" + currentUrl);
        // hangout[i].setAttribute("href", "https://plus.google.com/share?url=" + currentUrl);
        mailto[i].setAttribute("href", "mailto:?Subject= Tata.com url&Body=%20" + currentUrl);
...
function shareLabels() {
    if (window.innerWidth < 767) {
        var parallaxControls = document.querySelectorAll(".forStoryPages > .parallaxControls")[0];
        if (parallaxControls) {
            parallaxControls.classList.remove("parallaxControls"); // sonar changes
        }
    }
}
...
// Code for Video section banner video
//Load videos on about us page for all folds
//remove this because use aboutUsVideo.js code which is common
// function NeverRestVideoLoad() {
//     let neverRestVideos = document.getElementsByClassName("NeverRestBannerVideo");
//     for (var i = 0; i < neverRestVideos.length; i++) {
//         if (window.innerWidth > 991) {
//             if (neverRestVideos[i].hasAttribute("desktopVideo")) {
//                 neverRestVideos[i].children[0].setAttribute("src", neverRestVideos[i].getAttribute("desktopVideo"));
//                 neverRestVideos[i].load();
//                 playPauseNeverRestVideo();
//                 neverRestVideos[i].parentElement.querySelector(".img-responsive").classList.add("noDisplay");
//             }
//         } else if (window.innerWidth > 575) {
//             if (neverRestVideos[i].hasAttribute("tabVideo")) {
//                 neverRestVideos[i].children[0].setAttribute("src", neverRestVideos[i].getAttribute("tabVideo"));
//                 neverRestVideos[i].load();
//                 playPauseNeverRestVideo();
//                 neverRestVideos[i].parentElement.querySelector(".img-responsive").classList.add("noDisplay");
//             }
//         } else {
//             if (neverRestVideos[i].hasAttribute("mobileVideo")) {
//                 neverRestVideos[i].children[0].setAttribute("src", neverRestVideos[i].getAttribute("mobileVideo"));
//                 neverRestVideos[i].load();
//                 playPauseNeverRestVideo();
//                 neverRestVideos[i].parentElement.querySelector(".img-responsive").classList.add("noDisplay");
//             }
...
// //Play about us videos on folds based on current scroll position
// function playPauseNeverRestVideo() {
//     let bottomHalfHeight = (window.innerHeight*2)/10;
//     let neverRestVideos = document.getElementsByClassName("NeverRestBannerVideo");
//     for (var i = 0; i < neverRestVideos.length; i++) {
//         if (((window.innerWidth > 991) && (neverRestVideos[i].hasAttribute("desktopVideo")))
//             || ((window.innerWidth < 991 && window.innerWidth > 575) && (neverRestVideos[i].hasAttribute("desktopVideo")))
//             || ((window.innerWidth < 575) && (neverRestVideos[i].hasAttribute("desktopVideo")))
//         ) {
//             if ((neverRestVideos[i].getBoundingClientRect().top > bottomHalfHeight || neverRestVideos[i].getBoundingClientRect().top < (-1*bottomHalfHeight)) && neverRestVideos[i].paused === false) {
//                 neverRestVideos[i].pause();
//             }
//             else if (neverRestVideos[i].getBoundingClientRect().top <= bottomHalfHeight && neverRestVideos[i].getBoundingClientRect().top > (-1*bottomHalfHeight) && neverRestVideos[i].paused === true) {
//                 neverRestVideos[i].play();
//             }
//         }
//     }
// }

// document.addEventListener("scroll", playPauseNeverRestVideo);
// window.addEventListener("blur", function(){
//     let neverRestVideos = document.getElementsByClassName("NeverRestBannerVideo");
//     for (var i = 0; i < neverRestVideos.length; i++) {
//         if (((window.innerWidth > 991) && (neverRestVideos[i].hasAttribute("desktopVideo")))
...
//     }
// })
// window.addEventListener("focus", playPauseNeverRestVideo);

var slidesCarousel = document.querySelectorAll(".slidesCarousel");
if(slidesCarousel.length != 0){
var slideCarItem = document.querySelectorAll(".slidesCarousel .carItem");

var listItem = document.querySelectorAll(".slidesCarousel .slideNumList li");
var prevSlide = document.getElementsByClassName("prevSlide")[0];
// document.addEventListener('DOMContentLoaded', function () {
// if(slidesCarousel.length != 0 ){
// for (var i = 0; i < slideCarItem.length; i++){
// if(slideCarItem[i].querySelector(".slideImg") && slideCarItem[i].querySelector(".slideImg picture img").getAttribute('src').trim() == ''){
// slideCarItem[i].querySelector(".slideImg").remove();
// }
//             if(slideCarItem[i].querySelector(".slideImg") && slideCarItem[i].querySelector(".colorBGForSlide")){
// slideCarItem[i].querySelector(".colorBGForSlide").remove();
// }
            
//         }
//     }
// });
//for starting the slideshow when it will come in viewport
function isElementInViewport(el) {
    var rect = el.getBoundingClientRect();
    return (rect.top <= window.innerHeight) && ((rect.top + rect.height) >= 0);
}
document.addEventListener('DOMContentLoaded', function () {
    for (var i = 0; i < slidesCarousel.length; i++) {
        if (isElementInViewport(slidesCarousel[i])) {
            slidesCarousel[i].getElementsByClassName('slideShowIndicator')[0].classList.remove('expanded')
        }else{
            slidesCarousel[i].getElementsByClassName('slideShowIndicator')[0].classList.add('expanded')
        }
...
var slideshowItem = document.querySelectorAll(".slidesCarousel > .innerCarousel > .carItem");
var src, posterImage, videoDiv,aspectRatio;
var autoTrue = false;
src='';
for (var i = 0; i < slideshowItem.length; i++) {
    if (slideshowItem[i].children[0].children[0] && slideshowItem[i].children[0].children[0].classList.contains("videoContainerSlide")) {
        var currentSlide = slideshowItem[i]
        if (slideshowItem[i].classList.contains("active")) {
            autoTrue = true;
        } else {
            autoTrue = false;
        }
        slideshowItem[i].children[0].children[0].id = "divVideoslide" + i;
        if(window.innerWidth > 300 && window.innerWidth < 768){
            videoDiv = currentSlide.children[0].children[0].dataset;
            posterImage = videoDiv.imageMob;
            if(videoDiv.hrefMob && videoDiv.hrefMob.trim() != ''){
                src = videoDiv.hrefMob;
                aspectRatio = '9:16';
            }else{
                currentSlide.getElementsByClassName('slideVideo')[0].style.display = 'none';
                currentSlide.style.backgroundImage = "url('"+posterImage+"')";
                currentSlide.style.backgroundSize = "100% 100%";
            }
        }else if(window.innerWidth < 992){
            videoDiv = currentSlide.children[0].children[0].dataset;
            posterImage = videoDiv.imageTab;
            if(videoDiv.hrefTab && videoDiv.hrefTab.trim() != ''){
                src = videoDiv.hrefTab;
                aspectRatio = '3:4';
            }else{
                currentSlide.getElementsByClassName('slideVideo')[0].style.display = 'none';
                currentSlide.style.backgroundImage = "url('"+posterImage+"')";
            ...
            videoDiv = currentSlide.children[0].children[0].dataset;
            posterImage = videoDiv.imageDesktop;
            if(videoDiv.hrefDesktop && videoDiv.hrefDesktop.trim() != ''){
                src = videoDiv.hrefDesktop;
                aspectRatio = '16:9';
            }else{
                currentSlide.getElementsByClassName('slideVideo')[0].style.display = 'none';
                currentSlide.style.backgroundImage = "url('"+posterImage+"')";
        ...
        if(src != ''){
            var playerInstance = jwplayer(slideshowItem[i].children[0].children[0].id);
            playerInstance.setup({
                "skin": {
                    "name": "myskin"
                },
...
var navArrow = document.querySelectorAll(".slidesCarousel .navArrow");
if (navArrow) {
    for (var i = 0; i < navArrow.length; i++) {
        navArrow[i].addEventListener("touchstart", touchedStartA);
        navArrow[i].addEventListener("touchend", touchedEndA);
        navArrow[i].addEventListener("touchcancel", touchedEndA);
    }
}
function touchedStartA(e){
    //this.classList.toggle("buttonBackgroundHover");
this.classList.add("buttonBackgroundHover");
}
function touchedEndA(e){
    // this.classList.toggle("buttonBackgroundHover");
this.classList.remove("buttonBackgroundHover");
}

function preventDefaultSlide(e) {
    if(e.target.classList.contains('slideSection') || e.target.classList.contains('flexContent') || !e.target.closest('.carItem')){
        e.preventDefault();
    }else{
        if(e.target.closest('.carItem')){
            var slideContentDiv = e.target.closest('.carItem').getElementsByClassName("slideContentDiv")[0]
            var delta = e.wheelDelta ? e.wheelDelta : -e.deltaY;
            var isScrollingUp = delta > 0;
            var scrollTop = slideContentDiv.scrollTop;
            var scrollHeight = slideContentDiv.scrollHeight;
            var offsetHeight = slideContentDiv.offsetHeight;
            var canScrollUp = scrollTop > 0;
            var canScrollDown = scrollTop + offsetHeight < scrollHeight;
            if(scrollHeight <= scrollTop + offsetHeight){
                slideContentDiv.scrollTop -= 1;
            }
            if(scrollTop == 0){
                slideContentDiv.scrollTop = 1;
            }
            if((isScrollingUp && canScrollUp) || (!isScrollingUp && canScrollDown)){
                return;
            }
...
function disabletouch() {
    window.addEventListener('DOMMouseScroll', preventDefaultSlide, false); 
    window.addEventListener('touchmove', preventDefaultSlide, wheelOpt);
    window.addEventListener(wheelEvent, preventDefaultSlide, wheelOpt);
}
function enabletouch() {
    window.removeEventListener('DOMMouseScroll', preventDefaultSlide, false);
    window.removeEventListener('touchmove', preventDefaultSlide, wheelOpt);
    window.removeEventListener(wheelEvent, preventDefaultSlide, wheelOpt);
}
// //code for progress bar animations
var counter = 0;
var totalTime = 15000;
setInterval(function () {
    var activeSlide = document.querySelectorAll(".carItem.active")[0];
    var slideShowIndicator = document.getElementsByClassName("slideShowIndicator")[0];
    if((!slideShowIndicator.classList.contains("expanded") && !activeSlide.classList.contains("expanded"))){
        counter += 1;
    }
    var progressBarWidth = activeSlide.querySelectorAll('.slideSeekBar')[0].style.width
    if (parseInt(progressBarWidth) > 100) {
        activeSlide.querySelector('.slideSeekBar').style.width = 100 + "%";
    }
    else {
        activeSlide.querySelector('.slideSeekBar').style.width = counter + "%";
    }
