...
xhr.open("GET", "/bin/investorStockDataServlet?", true);
xhr.onload = function () {
    if (xhr.status === 200) {
        var response = JSON.parse(this.responseText).investorStockData;
        for (var i = 0; i < response.length; i++) {
            var companyElement = document.getElementsByClassName(
                response[i].co_code
            )[0];
            if (companyElement) {
                companyElement.querySelector(".blackColor span").innerHTML =
                    response[i].BSECode;
                companyElement.querySelectorAll(".NSE span")[0].innerHTML =
                    response[i].NSEPrice;
                var NSEPerChange = Math.round(response[i].NSEPerChange * 100) / 100;

                companyElement.querySelectorAll(
                    ".NSE span"
                )[1].innerHTML = NSEPerChange + "%";

                if (NSEPerChange < 0) {
                    companyElement.querySelector(".NSE").classList.add("neg");
                } else if (NSEPerChange > 0) {
                    companyElement.querySelector(".NSE").classList.add("pos");
                }
                companyElement.querySelectorAll(".BSE span")[0].innerHTML =
                    response[i].BSEPrice;
                var BSEPerChange = Math.round(response[i].BSEPerChange * 100) / 100;

                companyElement.querySelectorAll(
                    ".BSE span"
                )[1].innerHTML = BSEPerChange + "%";
                if (BSEPerChange < 0) {
                    companyElement.querySelector(".BSE").classList.add("neg");
                } else if (BSEPerChange > 0) {
                    companyElement.querySelector(".BSE").classList.add("pos");
                }
                companyElement.querySelector(".MCap").innerHTML = response[i].MCAP;
            }
            //if no NSE and BSE price in json it should display none
            if (!response[i].NSEPrice) {
            ...
            //NSE and BSE price on cards
            var NSEPerChange = Math.round(response[i].NSEPerChange * 100) / 100;
            var BSEPerChange = Math.round(response[i].BSEPerChange * 100) / 100;
            for (
                var j = 0;
                j < document.querySelectorAll(".C" + response[i].co_code).length;
                j++
            ) {
                document
                    .querySelectorAll(".C" + response[i].co_code)
                [j].querySelectorAll(" .nse span")[0].innerHTML =
                    response[i].NSEPrice;
                document
                    .querySelectorAll(".C" + response[i].co_code)
                [j].querySelectorAll(" .bse span")[0].innerHTML =
                    response[i].BSEPrice;
                document
                    .querySelectorAll(".C" + response[i].co_code)
                [j].querySelectorAll(" .nse span")[1].innerHTML = NSEPerChange + "%";
                document
                    .querySelectorAll(".C" + response[i].co_code)
                [j].querySelectorAll(" .bse span")[1].innerHTML = BSEPerChange + "%";

                if (BSEPerChange < 0) {
                    document
                        .querySelectorAll(".C" + response[i].co_code)
                    [j].querySelectorAll(" .bse")[0]
                        .classList.add("neg");
                } else if (BSEPerChange > 0) {
                    document
                        .querySelectorAll(".C" + response[i].co_code)
...
/*New code for Accessibility underlining hyperlinks*/
if (window.innerWidth < 992) {
    var blackColor = document.getElementsByClassName("blackColor");
    if (blackColor.length != 0) {
        for (var i = 0; i < blackColor.length; i++) {
            blackColor[i].addEventListener("touchstart", function (e) {
                e.target.style.borderBottom = "1px solid";
                if (e.target.tagName == "SPAN") {
                    e.target.parentElement.style.borderBottom = "1px solid";
                }
                console.log(e.target.tagName, e.target.parentElement)
            }, false);
            blackColor[i].addEventListener("touchend", function (e) {
                e.target.style.borderBottom = "none";
                if (e.target.tagName == "SPAN") {
                    e.target.parentElement.style.borderBottom = "none";
                }
            }, false);
            blackColor[i].addEventListener("touchcancel", function (e) {
                e.target.style.borderBottom = "none";
                if (e.target.tagName == "SPAN") {
                    e.target.parentElement.style.borderBottom = "none";
                }
