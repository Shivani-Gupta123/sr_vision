...
//facebook instragram twitter url declared in commonjs
var xhttp = new XMLHttpRequest();
...
xhttp.onload = function() {
  if (xhttp.readyState == 4 && xhttp.status === 200) {
    ...
    for (var i = 0; i < res.response.length; i++) {
      var leng = res.response.length;
      if (res.response[i].type == "twitter" && document.getElementsByClassName("feedsTwitter").length > 0) {
        for (var j = 0; j < res.response[i].feeds.length; j++) {
          document.getElementsByClassName("feedsTwitter")[j].innerHTML =
            res.response[i].feeds[j].content;
          document.getElementsByClassName("tweet-date")[j].innerHTML =
            res.response[i].feeds[j].friendlyTime;
          document
            .querySelectorAll(".twitterContent a")
            [j].setAttribute("href", res.response[i].feeds[j].link);
          if ((feedsTwitter[j].className = "feedsTwitter")) {
            var len = feedsTwitter[j].innerHTML.length;
            if (len >= 200) {
              feedsTwitter[j].innerHTML =
                feedsTwitter[j].innerHTML.substring(0, 200) + "...";
            }
          }
        }
      }
      if (leng > 1) {
        if (res.response[i].type == "facebook") {
          for (var j = 0; j < res.response[i].feeds.length; j++) {
            if (res.response[i].feeds[j].content) {
              if (res.response[i].feeds[j].content.length < 220) {
                document.querySelectorAll(".fB .feeds")[j].innerHTML =
                  res.response[i].feeds[j].content;
              } else {
                document.querySelectorAll(".fB .feeds")[j].innerHTML =
                  res.response[i].feeds[j].content.substring(0, 220) + "...";
              }
            }
            document.querySelectorAll(".fB .tweet-date")[j].innerHTML =
              res.response[i].feeds[j].friendlyTime;
            var JsonObj = res;
            if (JsonObj.response[1].feeds[0].media) {
              document.querySelectorAll(".fB .dynaImg")[j].src =
                res.response[i].feeds[j].media.url;
              document
                .querySelectorAll(".fB picture")
                [j].getElementsByTagName("source")[0].srcset =
                res.response[i].feeds[j].media.url;
              document
                .querySelectorAll(".fB picture")
                [j].getElementsByTagName("source")[1].srcset =
                res.response[i].feeds[j].media.url;
              document
                .querySelectorAll(".fB picture")
                [j].getElementsByTagName("source")[2].srcset =
                res.response[i].feeds[j].media.url;
            } else {
              document
                .querySelector(".fB picture img")
                .classList.remove("hide");
            }
          }
        }
        if (res.response[i].type == "instagram") {
          for (var j = 0; j < res.response[i].feeds.length; j++) {
            if(res.response[i].feeds[j].content){
            	if (res.response[i].feeds[j].content.length < 220) {
            	  document.querySelectorAll(".instagram .feeds")[j].innerHTML =
            	    res.response[i].feeds[j].content;
            	} else {
            	  document.querySelectorAll(".instagram .feeds")[j].innerHTML =
            	    res.response[i].feeds[j].content.substring(0, 220) + "...";
            ...
            document.querySelectorAll(".instagram .tweet-date")[j].innerHTML =
              res.response[i].feeds[j].friendlyTime;
            if (res.response[i].feeds[j].media.url != "undefined") {
              document.querySelectorAll(".instaGram .dynaImg")[j].src =
                res.response[i].feeds[j].media.url;
            } else {
              document
                .querySelectorAll(".instaGram picture img")
                .classList.remove("hide");
            }
          }
        }
...
var navActiveItem = document.querySelectorAll(".navText > a");
if (document.getElementsByClassName("activeMenu").length == 0) {
  navActiveItem[1].classList.add("activeMenu");
}

var dataAnalytics = document.getElementsByClassName("dataAnalytics");
for (i = 0; i < dataAnalytics.length; i++) {
  dataAnalytics[i].addEventListener("click", function() {
    dataLayer.push({
      event: "GAEvent",
      eventCategory: "Home Page Banner",
      eventAction: "Click",
...
//code for herobanner video
var heroBannerItem = document.querySelectorAll(
  ".heroCarousel > .innerCarousel > .carItem"
);

var src, posterImage, videoDivs;
var autoTrue = false;
for (var i = 0; i < heroBannerItem.length; i++) {
  if (heroBannerItem[i].children[2].classList.contains("videoContainerdiv")) {
    if (heroBannerItem[i].classList.contains("active")) {
      autoTrue = true;
    } else {
      autoTrue = false;
    }
    heroBannerItem[i].children[2].id = "divVideo" + i;
    var videoDiv = heroBannerItem[i].children[2].dataset;
    src = videoDiv.href;
    posterImage = videoDiv.image;
    var playerInstance = jwplayer(heroBannerItem[i].children[2].id);
    playerInstance.setup({
      skin: {
        name: "myskin"
      },
...
var youMightLike = document.getElementById("youMightLike");
if (youMightLike) {
  youMightLike.classList.add("parallaxWrapper");
}
var carousel=document.getElementsByClassName('carouselIndicators nIndicators');
        var carouselChildrens=carousel[0].children;
        for(let i=0;i<carouselChildrens.length;i++)
        {
            carouselChildrens[i].addEventListener('click',function(event){
                event.stopPropagation();
            })
        }
        function twitterCardClickable(event){
        
        event.preventDefault();
        //To check which part of card is clicked 
        if(event.target.classList.contains("newsFeedA") || event.target.classList.contains("feedsTwitter") ||event.target.classList.contains("tweet-date") ){
          var url= document.querySelector(".twitterFeed .carItem.active .twitterContent>a").getAttribute("href");        
          window.open(url);
        }
        else{
          var url = document.querySelector(".twitterFeed >a").getAttribute("href");
          window.open(url,"_self");
        }
    }
    
	let twitterCardJobs = [...document.querySelectorAll(".twitterFeed")].filter(x => x.querySelector(".jobsCarouselHome")); 
twitterCardJobs.map(x => x.addEventListener("click", (e)=>{
    if (!e.target.closest(".twitterContent, li")){
        window.open(x.lastElementChild.href, '_self');
    }}
    ));

if (window.innerWidth > 991) {
    document.addEventListener('DOMContentLoaded', () => {
        let linePointers = [...document.getElementsByClassName("linePointer")];
        let pointerTexts = [...document.getElementsByClassName("pointerText")];
        window.addEventListener("scroll",()=>{
            linePointers.forEach((x)=>x.classList.remove("artificialHover"));
            pointerTexts.forEach((x)=>x.classList.remove("artificialHover"));
        }, {once:true});
        for (let elem of linePointers) {
            setTimeout(() => { elem.classList.remove("artificialHover") }, 5000);
        }
        for (let elem of pointerTexts) {
            setTimeout(() => { elem.classList.remove("artificialHover") }, 5000);
        }
    })
}
var fullCardClick=document.getElementsByClassName("fullCardClick")[0];
fullCardClick.addEventListener('click',function(e){

    if(e.target.classList.contains("playVideo")){
 e.preventDefault();
    }
  else{
      let direction="_self";
    let toGetUrl=document.getElementsByClassName("CardUrl")[0].getAttribute('href');
  
      e.preventDefault();
    window.open(toGetUrl,direction);  
  }
...
if (document.querySelector("section.brandsSection")){
  var pageList;
  var currentPage;
  var numberPerPage;
  ...
  var listsArrya = [];
  var itemPushed = 0;
  var elementAdded = true;
  var currentSlide = 1;
  var scrollPer = 400;
  var srcDevice = "";
  if (window.innerWidth > 991){
    srcDevice = "srcdesktop";
  }
  ...
  var BrandSectionIndicator = document.getElementsByClassName("BrandSectionIndicator")[0];
  var indicatorsDiv = document.querySelectorAll(".forBrands");
  var events = Array.prototype.slice.call(document.querySelectorAll(".forBrands"));

  //this for getting total number of elements in carousel
  function getNumberOfPages() {
    return Math.ceil(events.length / numberPerPage);
  }

  //this for loading list in next page
  function nextPage() {
    currentPage += 1;
  ...
  //this for loading list in previous page
  function previousPage() {
    currentPage -= 1;
    loadList();
  }

  //this for loading list in first page
  function firstPage() {
    currentPage = 1;
    loadList();
  }

  //this for loading list in last page
  function lastPage() {
    currentPage = numberOfPages;
    loadList();
  }

  //this for loading list
  function loadList() {
      var begin = ((currentPage - 1) * numberPerPage);
      var end = begin + numberPerPage;
      var pageListChecked = document.querySelectorAll(".BrandSectionIndicator > .forBrands");
      if (pageList.length !== numberPerPage) {
          for (var i = 0; i < pageListChecked.length; i++) {
              if (!pageListChecked[i].classList.contains('hide')) {
                  pageList.push(pageListChecked[i]);
              }
          }
        // pageListChecked = [];
      }
      ...
      if((end - begin) == numberPerPage){
          if(events.slice(begin, end).length != numberPerPage){
            //sonar not used var
            // var currentElemLength = (events.slice(begin, end).length);            
              var newValue = events.slice(events.length - numberPerPage, events.length);
              pageList = [];
              pageList = newValue;
          }else{
  ...
  function drawList() {
    ...
    if (currentPage != 1) {
      ...
      for (var i = 0; i < events.length; i++) {
        if (!events[i].classList.contains("hide")) {
          nextElements.push(events[i]);
        }
      }
      if (nextElements.length < 7) {
        for (var i = itemPushed; i < numberPerPage - nextElements.length; i++) {
          events[i + nextElements.length].classList.remove("hide");
          events[i + nextElements.length].classList.add("slideLeft");
        }
        itemPushed += 5 - nextElements.length;
      }
    } else {
      itemPushed = 0;
    }
  ...
  function check() {
    if (window.innerWidth > 992) {
      var preDiv = document.getElementById("next");
      var nextDiv = document.getElementById("previous");
      if (preDiv || nextDiv) {
        if (currentPage != numberOfPages && currentPage == 1) {
          nextDiv.style.display = "block";
          preDiv.style.display = "none";
          return false;
        } else if (indicatorsDiv.length < numberPerPage) {
  ...
  var carItem = document.querySelectorAll(
    ".brandCarousel > .innerCarousel > .carItem"
  );

  function loadImage(i){
    if (!carItem[i].children[0].children[0].src.includes(carItem[i].children[0].children[0].dataset[srcDevice])){
      carItem[i].children[0].children[0].src = carItem[i].children[0].children[0].dataset[srcDevice];
    }
    if (!carItem[i].children[0].children[0].complete){
                
      let backdrop = document.querySelector(".brandCarousel .fadeGlobalSearch")
      let spinner = document.querySelector(".brandCarousel .modalGlobalSearch")
      backdrop.style.display = "block";
      spinner.style.display = "block";
      carItem[i].children[0].children[0].onload = ()=>{
        backdrop.style.display = "none";
        spinner.style.display = "none";
      }
      
      if (i < (carItem.length-1)){
        carItem[i+1].children[0].children[0].src = carItem[i+1].children[0].children[0].dataset[srcDevice];
      }    
      else{
        carItem[0].children[0].children[0].src = carItem[0].children[0].children[0].dataset[srcDevice];
      }
    }
  ...
  setInterval(function() {
    for (var i = 0; i < carItem.length; i++) {
      if (carItem[i].classList.contains("active")){
        loadImage(i);
      }
      if (
        carItem[i].parentNode.classList.contains("brandCarouselItems") &&
        elementAdded
      ) {
        listsArrya.push(carItem[i]);
      }
      if (
        carItem[i].classList.contains("active") &&
        clickedItem != true &&
        listsArrya.indexOf(carItem[i]) === currentPage * numberPerPage
      ) {
        clickedItem = true;
        ...
        carItem[i].classList.contains("active") &&
        listsArrya.indexOf(carItem[i]) === carItem.length - 1
      ) {
        setTimeout('lastIndexFunction()', 9000);
      }
    }
    elementAdded = false;
  }, 1000);

  var brandsThumbnail = document.getElementsByClassName("BrandSectionIndicator");

  var buttonControl = document.getElementsByClassName("thumbnailControls");
  brandsThumbnail[0].addEventListener("click",(e)=>{
    if (e.target.parentNode.getAttribute("data-slide-to")){
      let i = parseInt(e.target.parentNode.getAttribute("data-slide-to"));
      loadImage(i);
    }
  ...
  for (var i = 0; i < buttonControl.length; i++) {
    buttonControl[i].addEventListener("mouseenter", function() {
      this.childNodes[1].children[0].style.display = "block";
      this.childNodes[1].children[1].style.display = "none";
    });
    buttonControl[i].addEventListener("mouseleave", function() {
