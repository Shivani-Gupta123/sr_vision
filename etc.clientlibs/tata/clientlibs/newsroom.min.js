...
if (document.getElementsByClassName("activeMenu").length == 0) {
	navActiveItem[4].classList.add("activeMenu");
}
...
document.getElementsByClassName("select-selected")[0].innerHTML = "Popular";

var start = 0;
var count = 6;
var servletUrl = "/bin/newsroomServlet?";
var tag = "Popular";

var flagsImage = [false, false];
var flagsText = [false, false];

document.getElementsByClassName("viewMoreContent")[0].addEventListener("click", loadMore)

function imageCardShareFunction() {
    var shareContainer = document.createElement("div");
    shareContainer.classList = shareContainer;
    shareContainer.innerHTML = "<div class=\"shareContainer\">\
    <div>\
        <div class=\"Tooltip\">\
            <a class=\"shareBtnParent\">\
                <img src=\"/content/dam/tata/images/common/shareWhite.svg\" class=\"stIcons shareBtn normalImg\" alt=\"Share\">\
        ...
        <div class=\"shareOptions\">\
            ...
            <div class=\"mailto shareEvent\">\
                <a class=\"mailtoAnchor\" target=\"_blank\">\
                </a>\
            </div>\
        </div>\
    </div>\
...
function textCardShareFunction() {
    var shareContainer = document.createElement("div");
    shareContainer.classList = shareContainer;
    shareContainer.innerHTML = "<div class=\"shareContainer\">\
    <div>\
        <div class=\"Tooltip\">\
            <a class=\"shareBtnParent\">\
                <img src=\"/content/dam/tata/images/common/share.svg\" class=\"stIcons shareBtn normalImg\" alt=\"Share\">\
                <img src=\"/content/dam/tata/images/common/shareOptions/color/share.svg\" class=\"hideActive stIcons shareBtn\" alt=\"Share\">\
            </a>\
            <span class=\"tooltiptext hidden-sm hidden-xs\">Share</span>\
        </div>\
...
customSelect = document.getElementsByClassName("select-items");

for (var i = 0; i < customSelect.length; i++) {
    customSelect[i].addEventListener("click", function() {
        var selectedTag = document.getElementsByClassName("same-as-selected")[0].innerHTML;
        document.getElementById("newcards").innerHTML = "";
        document.getElementById("viewNext").classList.add("hide");
        if (selectedTag == "Popular") {
            document.getElementById("popularNews").classList.remove("hide");
        } else {
            document.getElementById("popularNews").classList.add("hide");
            for (var i = 0; i < flagsImage.length; i++)
                flagsImage[i] = false;
            for (var i = 0; i < flagsText.length; i++)
                flagsText[i] = false;
...
function loadMore() {
    ...
    if (tag == "Other") {
        tag = "Other";
        var tags = document.getElementsByClassName("select-items")[0];
        var tagsOmit = tags.children;

        for (var i = 1; i < tagsOmit.length; i++) {
            if (i < tagsOmit.length - 1)
                tag = "" + tag + tagsOmit[i].innerHTML + ",";
            else
                tag = "" + tag + tagsOmit[i].innerHTML + "";
        }
    }
    ...
    xhr.open('GET', servletUrl + "start=" + start + "&count=" + (count + 1) + "&tag=" + tag);
    xhr.onload = function() {
...
function createNews(response) {
    if (response.length < 7) {
        ...
        document.getElementById("viewNext").classList.remove("hide");
        response.pop();
    }

    for (var i = 0; i < response.length; i++) {
        if (response[i].desktopImagePath != "")
            createImageCard(response[i], i + start);
        else
            createTextCard(response[i], i + start);
    }


}

function createImageCard(card, i) {

    var a = Math.ceil(i / 3);
    if (i % 3 == 0) {
        var newsbox = document.createElement("div");
        newsbox.classList = "topMargin topMargin767";
        newsbox.setAttribute("id", "flex" + i / 3);
    } else {
        a--;
        var newsbox = document.getElementById("flex" + a);

    }
    var news = document.createElement("div");
    news.classList = "col-lg-4 col-md-4 col-sm-6 col-xs-6 width100 topMargin topMarginsm topMarginxs topMargin767 widthlands news module";

    var picture = document.createElement("picture");

    var src1 = document.createElement("source");
    src1.setAttribute("media", "(min-width: 991px)");
    src1.setAttribute("srcset", card.desktopImagePath);

    var src2 = document.createElement("source");
    src2.setAttribute("media", "(min-width: 575px)");
    src2.setAttribute("srcset", card.tabImagePath);

    var src3 = document.createElement("source");
    src3.setAttribute("media", "(min-width: 0px)");
    src3.setAttribute("srcset", card.mobileImagePath);

    var img = document.createElement("img");
    img.classList = "img-responsive backgImg imgCard";
    img.setAttribute("src", card.desktopImagePath);
    img.setAttribute("alt", card.title);

    var imgOverlay = document.createElement("div");
    imgOverlay.classList = "imageOverlay50";

    var hover = document.createElement("a");
    hover.classList = "hoverCard";
    hover.setAttribute("href", card.url);

    var article = document.createElement("div");
    article.classList = "whiteContent";

    var articleType = document.createElement("div");
    articleType.classList = "whiteCaption";
    articleType.innerHTML = card.tag.replace("-", " ");

    var articleTitle = document.createElement("h3");
    articleTitle.classList = "nHeadingW";
    articleTitle.innerHTML = card.title;

    var articleDescription = document.createElement("div");
    articleDescription.classList = "storyContent";

    var description = document.createElement("div");
    description.classList = "ymlFeedArticleW";
    var len = card.description.length;
    if (len >= 130) {
        card.description = card.description.substring(0, 130) + '...';
    }
    description.innerHTML = card.description;

    var border = document.createElement("div");
    border.classList = "feedArticleBorder borderAnimCards ";

    var share = imageCardShareFunction();

    article.appendChild(share);

    articleDescription.appendChild(description);
    articleDescription.appendChild(border);

    article.appendChild(articleType);
    article.appendChild(articleTitle);
    article.appendChild(articleDescription);

    picture.appendChild(src1);
    picture.appendChild(src2);
    picture.appendChild(src3);
    picture.appendChild(img);

    news.appendChild(picture);
    news.appendChild(imgOverlay);
    news.appendChild(article);
    news.appendChild(hover);

    if (i % 3 == 0) {
        newsbox.appendChild(news);
        document.getElementById("newcards").appendChild(newsbox);
    ...
    shareImage(news, card.url, card.description, card.hashtags);
    //console.log(card);

}

function createTextCard(card, i) {
    ...
    var articleType = document.createElement("div");
    articleType.classList = "blueCaption";
    articleType.innerHTML = card.tag.replace("-", " ");

    var articleTitle = document.createElement("h3");
    articleTitle.classList = "nHeading";
    articleTitle.innerHTML = card.title;
    ...
    var description = document.createElement("div");
    description.classList = "ymlTextArticle";
    var len = card.description.length;
    if (len >= 130) {
        card.description = card.description.substring(0, 130) + '...';
    }
    ...
    titleContent.appendChild(articleType);
    titleContent.appendChild(articleTitle);
    titleContent.appendChild(articleDescription);

    var share = textCardShareFunction();
    titleContent.appendChild(share);

    news.appendChild(titleContent);
    news.appendChild(hover);
    ...
    shareTextCard(news, card.url, card.description, card.hashtags);

}

function shareImage(newsbox, shareUrl, shareText, twitterHashtags) {
    var fb = newsbox.querySelectorAll(".fb > .fbAnchor");
    var twitter = newsbox.querySelectorAll(".twitter > .twitterAnchor");
    var linkedin = newsbox.querySelectorAll(".linkedin > .linkedinAnchor");
    var hangout = newsbox.querySelectorAll(".hangout > .hangoutAnchor");
    var mailto = newsbox.querySelectorAll(".mailto > .mailtoAnchor");
    for (var i = 0; i < fb.length; i++) {
        if (fb || twitter || linkedin || hangout || mailto) {
            ...
            if (hangout[i]) {
                ...
                function chngImghangout() {
                    this.style.backgroundImage = "url(/etc.clientlibs/tata/clientlibs/assets/resources/img/common/shareOptions/G+white.svg)";
                }
            }
            if (mailto[i]) {
                mailto[i].addEventListener("mouseenter", chngImgColormailto);
            ...
            var twitterShareParams = "";
            if (shareText !== "") {
                twitterShareParams += "&text=" + encodeURIComponent(shareText);
            }
            if (twitterHashtags !== "") {
                twitterShareParams += "&hashtags=" + twitterHashtags;
            }
            var currentUrl = shareUrl;
            currentUrl = currentUrl.replace("content/tata/en/", "");
            fb[i].setAttribute("href", "https://www.facebook.com/sharer/sharer.php?u=https://" + window.location.host + currentUrl);
            twitter[i].setAttribute("href", "https://twitter.com/share?url=https://" + window.location.host + currentUrl + twitterShareParams);
            linkedin[i].setAttribute("href", "https://www.linkedin.com/sharing/share-offsite/?url=https://" + window.location.host + currentUrl);
            //hangout[i].setAttribute("href", "https://plus.google.com/share?url=https://" + window.location.host + currentUrl);
            mailto[i].setAttribute("href", "mailto:?subject= Tata.com url&body=%20https://" + window.location.host + currentUrl);
            var isMobile = {
                Android: function() {
                    return navigator.userAgent.match(/Android/i);
                },
            ...
            var whatsApp = newsbox.getElementsByClassName("whatsApp");
            for (var j = 0; j < whatsApp.length; j++) {
                whatsApp[j].addEventListener("click", function() {
                    if (isMobile.any()) {
                        var whatsapp_url = "whatsapp://send?text=" + currentUrl;
    ...
    var shareBtnParent = newsElements.getElementsByClassName("shareBtnParent");
    var sharePanelOpen = false;
    ...
    if (shareBtnParent && sharePanelOpen == false) {
        for (var i = 0; i < shareBtnParent.length; i++) {
            if (flagsImage[i] != true) {
                flagsImage[i] = true;
                shareBtnParent[i].addEventListener("click", sharePanelOpenfn);
            }
        }
...
var newsElements = document.getElementById("filterSection");

function shareTextCard(newsbox, shareUrl, shareText, twitterHashtags) {
    var fb = newsbox.querySelectorAll(".fb > .fbAnchor");
    var twitter = newsbox.querySelectorAll(".twitter > .twitterAnchor");
    var linkedin = newsbox.querySelectorAll(".linkedin > .linkedinAnchor");
    var hangout = newsbox.querySelectorAll(".hangout > .hangoutAnchor");
